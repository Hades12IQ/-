...
The published build-3 APK passed its checksum and APK signature verification, but installing it over build 2 failed with `INSTALL_FAILED_UPDATE_INCOMPATIBLE`. Both CI logs confirmed that the old signing-cache path did not exist and no key was saved. Signing now uses an encrypted required GitHub secret and a dedicated private key kept outside source control; it fails closed rather than silently creating a different key. This secures future update continuity but cannot recover the lost keys of previews 2/3 or make those installed packages accept the new key.
