...
///
/// It also never cancels server work. `stop()` means "stop watching" — leaving a screen, changing
/// identity, tearing the app down. Ending the *job* is a separate, explicit act that goes through
...
final class JobWatcher {
    ...
    /// Run by the loop task as it ends.
    ///
    /// Not every exit from `run()` goes through `stop()` — a stream the transport cancelled ends
    /// the loop while the watcher is still neither stopped nor finished. Leaving the handle behind
    /// would make `poke()` a no-op (`start()` refuses while `loop != nil`) and the job would never
    /// be read again until the app relaunched.
    private func clearLoop() {
        loop = nil
    }

    /// Stops **watching**. The pointer and the server-side job are untouched.
    func stop() {
        isStopped = true
        loop?.cancel()
        loop = nil
    ...
    /// call this as often as they like; the loop takes at most one extra read from it, and it takes
    /// that read within a tenth of a second rather than at the end of the current cadence step.
    func poke() {
    ...
    /// How finely a wait is chopped so `poke()` is felt. Small enough that returning to the app
    /// reads the answer on the next frame, large enough that a dozen watchers cost nothing.
    private static let wakeGranularity: TimeInterval = 0.1
    ...
    ///
    /// The alternative — one sleep resolved by a continuation — is a hair cheaper and one missed
    ...
    private func idle(_ seconds: TimeInterval) async {
        ...
        while !isStopped, !isFinished, !Task.isCancelled {
            ...
            await JobClock.rest(Swift.min(Self.wakeGranularity, remaining))
        }
    }
}
