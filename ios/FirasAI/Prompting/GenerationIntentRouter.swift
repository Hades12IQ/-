...
enum GenerationIntentRouter {
    ...
    static func classify(_ text: String, history: [ChatMessage], api: APIClient) async -> Decision? {
        guard !text.isEmpty, text.utf16.count <= 12_000 else { return nil }
        if isFinishedImagePrompt(text) { return Decision(kind: "image", requirements: "", codeTarget: "unknown", codeLanguage: "unknown") }
        struct Context: Encodable {
            let product = "chat"
            let attached_image = false
        ...
        guard let data = try? JSONEncoder().encode(Input(request: text, context: Context(recent_messages: recent))),
