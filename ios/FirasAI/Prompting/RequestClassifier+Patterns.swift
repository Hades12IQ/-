...
extension RequestClassifier {
    ...
    static let songAboutPattern =
        ...
        /* **[fix]** MUSIC AS THE SUBJECT, NOT AS THE THING. Widening the make list to «اريد|ابي|
           بدي|i want» and the noun list to a bare «موسيقى|music» put the two inside the sixteen-
           character window of every request whose TOPIC is music but whose deliverable is not:
           «اريد كتاب عن الموسيقى», «اريد رواية عن الموسيقى», «ابي ملخص عن الاغاني العراقية»,
           "i want a book about music", "give me a lesson about songs" — every one of them
           routed to the music engine, which spent the reader's quota composing something nobody
           asked for. Naming the missing deliverables one at a time cannot close this; the
           grammar can. A song word that is the OBJECT of «عن»/«حول»/"about" is what the answer
           is ABOUT, and a request for a song never puts it there: «أغنية عن أمي» and "a song
           about my mother" name the song FIRST and its subject after it, so both survive. The
           leading boundary keeps «معنى» and «يعني» from supplying the «عن». */
        + "|(?:^|[\\s،,.:؛!?؟])(?:عن|حول|بخصوص)\\s*(?:ال)?(?:أغني|اغني|أغاني|اغاني|موسيق|نشيد|أناشيد|اناشيد|لطمي)"
        + "|\\babout\\s+(?:the\\s+)?(?:music|songs?|nasheed|anthem|lyrics)\\b"
