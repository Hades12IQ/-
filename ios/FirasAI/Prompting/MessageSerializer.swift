...
enum MessageSerializer {
    ...
    ///   1. rows are keyed by `cid`; the server copy wins when its content is longer (the worker
    ///      wrote the whole answer while this device only saw part of the stream);
    ...
    static func merge(local: [ChatMessage], server: [ChatMessage]) -> [ChatMessage] {
        var serverByCid: [String: ChatMessage] = [:]
        for m in server {
            if let cid = m.cid, !cid.isEmpty { serverByCid[cid] = m }
        }
        ...
        for m in local {
            if let cid = m.cid, !cid.isEmpty, let s = serverByCid[cid] {
                consumed.insert(cid)
        ...
        for m in server {
            if let cid = m.cid, !cid.isEmpty {
                if consumed.contains(cid) { continue }
                consumed.insert(cid)
                out.append(m)
            } else {
