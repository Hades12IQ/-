...
struct CodeCard: View {
    ...
    private var canPreview: Bool {
        ...
        switch (meta.lang ?? "").lowercased() {
        case "html", "htm", "svg", "xml": return true
    ...
    /// The fence's own `label` when it has one, else the language tag in the web's casing.
    private var languageLabel: String {
        if let title = meta.title, !title.isEmpty, meta.name != nil { return title }
        let key = (meta.lang ?? meta.ext ?? "").lowercased()
        guard !key.isEmpty else { return "CODE" }
        switch key {
