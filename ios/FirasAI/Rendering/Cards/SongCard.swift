    ...
    var header: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                /* NOT THE REQUEST, TWICE. `styleLine` falls back to the prompt when the engine
                   has named no genre, and while a song is being written the prompt IS the title,
                   so the card printed the reader's sentence and then printed it again underneath.
                   The genre is worth saying once the song exists; until then the working line
                   below says everything there is to say. */
                if resolvedPhase == .ready, !styleLine.isEmpty, styleLine != title {
                    Text(styleLine)
