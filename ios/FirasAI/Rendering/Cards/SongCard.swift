...
struct SongCard: View {
    ...
    func renderingRow(elapsed: Int) -> some View {
        if elapsed > SongCard.renderCeiling {
            noticeBody(
    ...
    /// Ten minutes — the web's `MUSIC_JOB_MAX_MS`. A song is about seventy seconds; anything still
    /// unfinished after ten is not coming back.
    static let renderCeiling: Int = 10 * 60
    ...
    var resolvedPhase: Phase {
        ...
        case .auto:
            ...
            if startedAt != nil { return .rendering }
            return .pending
