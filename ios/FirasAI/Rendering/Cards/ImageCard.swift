...
/// | downloading | the same cover, one honest word: «أنزّل الصورة…» | 300 pt, square |
...
///
/// Four defects the owner hit on a device are fixed here and each one is a rule now:
///
...
/// 4. **The machine prompt is not body text.** The web prints it nowhere on a finished or failed
///    card — after the rewriter it is «a wall of machine text nobody wrote» (`app.js:6209-6212`) —
///    and the owner's screenshot had that English wall under an Arabic error. It earns its place in
///    exactly one state: while the render is running and nothing else says which picture this is.
struct ImageCard: View {
    ...
    /// The same plate, one honest word. This branch is reached only when the fence already carries a
    /// cache key — the picture exists on the server and the bytes are on their way — so telling the
    /// reader «أُركّب المشهد» here would be a lie, and a blank box would be the defect the cover was
    /// written to remove.
    private var downloadingCover: some View {
        cover(ratio: 1, words: [Strings.Media.fetchingImage(lang)])
    }
    ...
    /// The request, while the request is all there is.
    ///
    /// **Only while the render is running.** The web prints the prompt on no card at all — it is
    /// machine-rewritten English and `buildImageCard` says so in as many words (`app.js:6209-6212`)
    /// — and the owner's screenshot had exactly that English wall sitting under an Arabic download
    /// error, which is the worst place it could possibly be. A card that is downloading, has failed
    /// or is finished shows the picture and the reader's own caption and nothing else.
    @ViewBuilder
    private var promptLine: some View {
        if showsPrompt, showsRequestLine, !promptText.isEmpty {
            Text(promptText)
                .font(FirasType.caption)
    ...
    /// The one phase in which the machine's English prompt is worth reading: the render is running
    /// (or has run too long and is offering a retry), there is no picture yet, and nothing else on
    /// the card says which picture this is. Downloading, failed and finished all hide it, which is
    /// what the web does on every card it draws (`app.js:6209-6212`).
    private var showsRequestLine: Bool {
