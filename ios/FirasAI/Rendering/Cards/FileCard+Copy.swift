...
extension FileCard {
    ...
    ///
    /// The extension is appended here because the model is instructed to write the filename
    /// **without** one ("SHORT MEANINGFUL NAME in the user's language, NO file extension" —
    /// `web-prompt-builder.md §A.6`), and a file card whose name is «تقرير الجاذبية» with nothing
    /// after it does not read as a file at all.
    var displayName: String {
        let ext = FileCardKind.normalised(meta.format)
        if let name = meta.name?.trimmingCharacters(in: .whitespacesAndNewlines), !name.isEmpty {
            return FileCard.withExtension(name, ext)
        }
        if let title = meta.title?.trimmingCharacters(in: .whitespacesAndNewlines), !title.isEmpty {
            return FileCard.withExtension(title, ext)
        }
        return FileCardKind.fallbackName(meta.format)
    }

    /// `name.ext`, unless the name already ends in it or there is no extension to add.
    static func withExtension(_ name: String, _ ext: String) -> String {
        guard !ext.isEmpty else { return name }
        if name.lowercased().hasSuffix("." + ext) { return name }
        return name + "." + ext
    }
...
enum FileCardCopy {
    ...
    /* **[new]** — THE ONE LINE THAT MAKES THE CARD HONEST.
       The web never needed it: there, the card is written when the file already exists. Here the
       metadata block arrives first and the design streams after it, so the plate is on screen for
       a minute before there is anything to open. This is what it says for that minute — no
       promise, no percentage, no verb that claims the file is nearly there. Just where it is. */
    static let preparing = LText(
        ar: "الملف قيد التحضير…",
        en: "The file is still being prepared…"
    )
