...
///
/// One rule the picture card learned the hard way and this card keeps: **a resolver that answers
/// `nil` has not failed.** It answers `nil` while the store is fetching the very same file, which is
/// what the store does the instant a render lands, and a clip is the biggest of the three files —
/// so the first ask comes back empty essentially always. The card asks again on a backoff for two
/// minutes before it will call a download failed.
struct VideoCard: View {
    ...
    /// The clip exists — the fence carries its key — and its file is on the way. Saying «يولّد
    /// الفيديو…» here would be a lie, and an empty box would be the defect the cover exists to
    /// remove, so the plate stays and only the word changes.
