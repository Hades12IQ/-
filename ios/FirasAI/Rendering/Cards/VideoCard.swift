...
///
/// Three rules the picture card learned the hard way and this card now keeps too:
///
/// 1. **A resolver that answers `nil` has not failed.** It answers `nil` while the store is fetching
///    the very same file, which is what the store does the instant a render lands, and a clip is the
///    biggest of the three files — so the first ask comes back empty essentially always. The card
///    asks again on a backoff for two minutes before it will call a download failed.
/// 2. **The caption is the reader's, or there is no caption.** This card printed `meta.prompt` under
///    every clip, in every state — and on a video fence that field is the English shot description
///    the app wrote for Wan-3: «a single continuous push-in on a lone fisherman at dawn, warm rim
///    light…». It was under the cover while the clip rendered, under the poster when it finished and
///    under the error when it failed, and nobody had ever audited this card the way the song card
///    was audited. Only `note`, which the reader writes, is drawn now.
/// 3. **A clip already on this device does not wear a render cover.** Finding a file in a folder
///    takes a frame or two, and putting the animated plate on screen for them is what made
///    reopening an old conversation look like the app generating it all over again.
struct VideoCard: View {
    ...
    /// The first ask for the file came back empty, so the wait is real and worth naming.
    ...
    private var cover: some View {
        plate(word: VideoCardCopy.working(lang))
    }

    /// The clip exists — the fence carries its key — and its file is being found or fetched.
    ///
    /// Silent while that is merely a look in a folder, which is the whole of it for a clip this
    /// device already holds. Saying «يولّد الفيديو…» here would be a lie in any case; saying
    /// «أنزّل المقطع…» before an ask has actually come back empty is a smaller one, and it is the
    /// one that made every scroll back look like a fresh render.
    @ViewBuilder
    private var downloadingCover: some View {
        if fetchIsSlow {
            plate(word: Strings.Media.fetchingClip(lang))
        } else {
            MediaCoverPlate(
                palette: palette,
                lang: lang,
    ...
    private func plate(word: String) -> some View {
        MediaCoverPlate(
            palette: palette,
            lang: lang,
            motionOn: motionOn,
    ...
    private var captionRow: some View {
        if !captionText.isEmpty || fileURL != nil || shareURL != nil {
            captionRowBody
        }
    }

    private var captionRowBody: some View {
        HStack(spacing: 2) {
    ...
    private func load() async {
        ...
        var backoff = Backoff(initial: 0.5, max: 5)
        let deadline = Date().addingTimeInterval(VideoCard.fetchBudget)
        var resolved: URL?

        while !Task.isCancelled, resolved == nil {
            if let url = await resolveFile(meta) {
                resolved = url
                break
            }
        ...
        guard let url = resolved else {
            // Cancellation is a row that scrolled away, not a failed download.
    ...
    /// Two minutes of asking. A clip is tens of megabytes and the store is very often fetching the
    /// same file at the same moment, so anything shorter reports a healthy download as a failure.
    private static let fetchBudget: TimeInterval = 120
    ...
    /// **The reader's caption, or nothing.** `meta.prompt` on a video fence is the English shot
    /// description written for the engine, and falling back to it put that paragraph under every
    /// clip in the app. A clip with no caption is a clip with no caption.
    private var captionText: String {
        String((meta.note ?? "").trimmingCharacters(in: .whitespacesAndNewlines).prefix(160))
    }
