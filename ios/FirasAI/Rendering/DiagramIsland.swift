...
///
/// Every figure draws on sight, exactly as `plotifyCodeBlock` does (`app.js:12904`) — including the
/// implicit 3D solids the prompt advertises by name (`x^2 + y^2 + z^2 = 9`, `z^2 = x^2 + y^2`). The
/// heaviest of them is a few hundred milliseconds of arithmetic, it runs inside the web view rather
/// than on the main actor, and it cannot make the transcript stutter — so a figure the reader asked
/// for is never held back behind a button.
///
...
struct DiagramIsland: UIViewRepresentable {
    ...
    func updateUIView(_ view: WKWebView, context: Context) {
        ...
        guard coordinator.loadedIdentity != identity else { return }
