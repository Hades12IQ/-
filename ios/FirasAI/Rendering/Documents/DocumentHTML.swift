...
enum DocumentHTML {
    ...
    private static func mathScript() -> String {
        ...
        out += "    try {\n"
        out += "      katex.render(tex, node, { displayMode: display, throwOnError: false, strict: false });\n"
        out += "    } catch (e) {\n"
    ...
    private static func tableOfContents(_ blocks: [MDBlock], arabic: Bool) -> String {
        ...
        var index = 0
        for block in blocks {
            guard case .heading(let level, let text) = block, level >= 2, level <= 3 else { continue }
            let plain = String(text.characters).trimmingCharacters(in: .whitespacesAndNewlines)
            guard !plain.isEmpty else { continue }
            index += 1
            rows.append(
                "<li class=\"doc-toc__item doc-toc__item--l\(level)\">"
