    ...
    private func capture(
        ...
        for frame in frames {
            ...
            if !ready {
                ...
                if let second = await snapshot(view, configuration: configuration), Self.hasInk(second) {
                    bitmap = second
                    ready = true
                }
            }
            guard ready else {
                attempted.remove(Self.key(items[frame.index].id, style))
                continue
            }
