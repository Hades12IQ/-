...
final class MathIsland {
    ...
    private static func hasInk(_ image: UIImage) -> Bool {
        ...
        /* MEASURED ON THE ALPHA CHANNEL, not on colour uniformity.
           The first version of this compared a 16x16 downscale against its own corner pixel,
           which was the right test for an OPAQUE page: ink is dark, the ground is light, the
           corner is ground. The page paints no ground any more, and that changes the question
           entirely: a blank frame is now fully TRANSPARENT, and a real equation is a scattering
           of opaque strokes over transparency. Downscaling a thin stroke to a sixteenth of its
           size averages it away, so the old test could refuse a perfectly good equation - and a
           refusal, before this, was a bitmap accepted with nothing visible in it.
           Alpha coverage is exact and cheap. The image is sampled on a grid at NATIVE
           resolution, so a hairline is either hit or it is not; a single opaque sample is proof
           of ink, and none at all is proof of a blank. Anything unmeasurable is accepted, since
           refusing on a failure to look would throw away good glyphs on no evidence. */
        let width = cg.width
        ...
        let drawn: Bool = pixels.withUnsafeMutableBytes { raw -> Bool in
            ...
            /* NEAREST NEIGHBOUR, DELIBERATELY. Interpolation is what averaged a stroke into
               nothing; sampling picks whole pixels and either lands on ink or misses it. */
        ...
        while index < pixels.count {
            if pixels[index] > 24 { opaque += 1 }
            index += 4
        }
        if opaque > 0 { return true }

        /* NO ALPHA AT ALL CAN ALSO MEAN AN OPAQUE BITMAP, which is what a page drawn before the
           ground was removed produces, and what some snapshot paths still hand back. Fall back
           to the old question for that case: does this frame contain more than one colour? */
        var index2 = 0
        let red = Int(pixels[0]), green = Int(pixels[1]), blue = Int(pixels[2])
        while index2 + 2 < pixels.count {
            let delta = abs(Int(pixels[index2]) - red)
                + abs(Int(pixels[index2 + 1]) - green)
                + abs(Int(pixels[index2 + 2]) - blue)
