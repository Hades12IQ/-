...
extension DiagramRuntime {
    ...
    (function (FD) {
      ...
      /* ── ENTRY POINT ────────────────────────────────────────────────────────────────────── */
      /* THE LABEL BESIDE THE DRAWING, in the notation a reader expects. The website hands its
         legend to KaTeX and shows `y = x²`; this island had no typesetter and printed the raw
         source, so a figure came with `y = x^2` and `sqrt(x)` stuck on beside it — which is
         exactly what "يضيف من يمه" describes. KaTeX is not loaded here on purpose (a second
         network dependency for one line of text is a bad trade), so the conversion is to Unicode:
         it covers what a plot legend actually contains — powers, roots, fractions of constants,
         Greek letters and the operators — and anything it does not know is left legible rather
         than mangled. Same rule the app’s offline math path already follows. */
      var GREEK = { alpha: "α", beta: "β", gamma: "γ", theta: "θ", phi: "φ",
        lambda: "λ", mu: "μ", omega: "ω", sigma: "σ", delta: "δ", pi: "π" };
      var SUP = { "0": "⁰", "1": "¹", "2": "²", "3": "³", "4": "⁴",
        "5": "⁵", "6": "⁶", "7": "⁷", "8": "⁸", "9": "⁹",
        "+": "⁺", "-": "⁻", "(": "⁽", ")": "⁾", n: "ⁿ" };
      var pretty = function (raw) {
        var t = String(raw == null ? "" : raw);
        if (!t) { return ""; }
        t = t.replace(/\*\*/g, "^");
        /* A superscript run only when EVERY character has a superscript form; a partial one would
           read worse than the caret it replaced. */
        t = t.replace(/\^\{([^{}]{1,8})\}|\^(-?[0-9n]{1,3})/g, function (whole, braced, bare) {
          var body = braced != null ? braced : bare;
          var out = "";
          for (var i = 0; i < body.length; i++) {
            var ch = SUP[body.charAt(i)];
            if (!ch) { return whole; }
            out += ch;
          }
          return out;
        });
        t = t.replace(/\bsqrt\s*\(([^()]*)\)/g, "√($1)");
        t = t.replace(/\babs\s*\(([^()]*)\)/g, "|$1|");
        t = t.replace(/\b(alpha|beta|gamma|theta|phi|lambda|mu|omega|sigma|delta|pi)\b/g,
          function (whole, name) { return GREEK[name] || whole; });
        t = t.replace(/\binf(inity)?\b/g, "∞");
      ...
      var legend = function (stage, p) {
        ...
        rows.forEach(function (f) {
          ...
          lb.textContent = f.surf || f.points ? pretty(f.expr) : "y = " + pretty(f.expr);
          row.appendChild(sw);
          row.appendChild(lb);
          box.appendChild(row);
        });
