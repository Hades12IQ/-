...
/// JavaScript, so there is exactly one grammar. This type only classifies, which is enough to pick
/// the card's title and to keep the one genuinely expensive mode behind a Run button.
struct DiagramSpec: Sendable, Hashable, Identifiable {
    ...
    /// The one mode worth a Run button. An implicit 3D solid is drawn by bisecting for the roots of
    /// `h(x,y,z)` at every point of a 56×56 mesh, 40 z-samples with 22 bisections each
    /// (`implicitSurfaceSheets`, `app.js:8482`): millions of evaluations, and the auto-fit pass runs
    /// the whole thing twice. Every other mode is a few thousand samples and draws on sight.
    var needsRunButton: Bool {
        mode == .implicitSurface
    }
    ...
    static func classify(_ body: String) -> Mode {
        ...
        for line in lines {
            ...
            if let head = plainHead(assignment.lhs) {
                // `y: -3..3`, `x = -4..4`, `theta: 0..2*pi` — a domain, never a curve.
