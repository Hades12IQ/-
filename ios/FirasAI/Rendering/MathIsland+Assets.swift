...
final class MathIslandAssets: NSObject, @preconcurrency WKURLSchemeHandler {
    ...
    /* THE BUNDLE COMES FIRST. `Resources/KaTeX/` carries the whole pinned build, so the common
       path never leaves the device: no latency, no cooldown, no first-run failure, and math that
       works on a plane. The Xcode target uses a synchronized group, so these files are copied
       into the bundle root by folder name; both lookups below are tried because a flat copy and
       a folder reference put them in different places and neither is worth a build-setting
       argument. The CDN stays as the fallback for a face the bundle somehow lacks. */
    private nonisolated static func bundled(_ path: String) -> Data? {
        let name = (path as NSString).lastPathComponent
        let stem = (name as NSString).deletingPathExtension
        let ext = (name as NSString).pathExtension
        guard !stem.isEmpty, !ext.isEmpty else { return nil }
        let folders = ["KaTeX/fonts", "KaTeX", nil] as [String?]
    ...
    private nonisolated static func fetch(session: URLSession, path: String) async -> Data? {
        if let local = bundled(path) { return local }
    ...
    func webView(_ webView: WKWebView, start urlSchemeTask: any WKURLSchemeTask) {
        ...
        /* Answered from the bundle on the same turn of the run loop — no task, no await, no
           chance for the page to be torn down while a font is still in flight. */
        if let local = Self.bundled(remote) {
            memory[remote] = local
            respond(key, urlSchemeTask, url: url, data: local, mime: Self.mime(for: remote))
            return
        }
    ...
    private func respond(
        ...
        active.remove(key)
        /* THE FONTS WERE BEING BLOCKED, and that is why the equations never came out right.
           A bare `URLResponse` carries NO HEADERS. A stylesheet fetched over a custom scheme does
           not care, but a font requested by `@font-face` is a CORS request, and WebKit treats a
           custom scheme as an opaque origin: with no `Access-Control-Allow-Origin` in the reply,
           every one of KaTeX's twenty faces is refused. The stylesheet loads, the script runs, the
           layout is computed — and then it is drawn in a fallback font, so an integral sign is a
           box and a fraction is a pile of digits. Nothing errors; it just looks broken.
           `HTTPURLResponse` lets the headers be stated, which is all this ever needed. */
        let encoding: String? = mime.hasPrefix("text/") ? "utf-8" : nil
        var headers = [
            "Content-Type": mime + (encoding == nil ? "" : "; charset=utf-8"),
            "Content-Length": String(data.count),
            "Access-Control-Allow-Origin": "*",
            "Cache-Control": "max-age=31536000",
        ]
        if mime.hasSuffix("woff2") || mime.hasSuffix("woff") || mime.hasSuffix("ttf") {
    ...
    (function () {
      ...
      window.firasRun = function (payload) {
        try {
          ...
          /* THE THEME'S GROUND, PAINTED. A transparent page was the better idea and the
             worse outcome: `takeSnapshot` is not documented to preserve alpha, WebKit is
             free to composite onto the view's own background, and a frame that comes
             back uniformly transparent is refused as a blank — which is why the
             equations stopped coming back after a restart. An opaque page is known to
             produce visible ink, and a faint rectangle on a card is a blemish where an
             equation that never renders is a broken feature. */
