...
enum MarkdownBlocks {
    ...
    /// How many times one `split` may scan for unfenced source before giving up on the idea.
    /// A file opens the answer; by the seventh candidate line this is reading prose.
    static let bareCodeAttemptLimit = 6
    ...
    static func split(_ markdown: String, streaming: Bool) -> [String] {
        ...
        var chunks: [String] = []
        var i = 0
        // A failed bare-code scan reads to the end of the answer, so an answer that quotes a
        // dozen `const …` lines in prose could pay for a dozen of them on every frame of the
        // stream. A real file opens once, at the top, and wins on the first attempt.
        var bareCodeAttempts = 0
        while i < lines.count {
            let line = lines[i]
            if isBlank(line) {
                i += 1
            ...
            if let fence = fenceOpen(line) {
                ...
                if let end = closeIndex {
                    ...
                    /* AN OPEN FENCE IS ALREADY CODE, and closing it for the parser is what puts
                       the listing in its box from the first line.
                       This handed the run back unclosed while streaming, so it parsed as PROSE:
                       the reader watched a program arrive as paragraphs and become code only
                       when the closing line landed. The opening line says ```python, which is
                       not ambiguous and does not get truer later.
                       The marker is added to this CHUNK and never to the message, so nothing the
                       reader copies or that is saved changes - only the box is drawn around what
                       has arrived, and it grows with it.
                       A `firas-*` card fence mid-stream is unaffected: the turn view answers an
                       open one with its own activity line before this text is parsed at all. */
            ...
            /* SOURCE THAT NOBODY FENCED. Read `bareCodeRun` before changing this: the code route
               asks the model for raw source with no fences, so a whole document arrives here with
               nothing for the scanner to catch and used to leave as paragraphs.

               It sits ABOVE every test but the fence on purpose, because a source file's lines LOOK
               like markdown and each of those tests would eat the file a line at a time before this
               could see it: `# -*- coding: utf-8 -*-` is a heading, `---` is a rule, `* p` is a list
               item, `<td>a|b</td>` is a table row. A real fence still outranks it, and the run stops
               at the next one, so nothing that WAS fenced changes shape. */
            if bareCodeAttempts < MarkdownBlocks.bareCodeAttemptLimit, bareCodeOpens(line) {
                bareCodeAttempts += 1
                if let run = bareCodeRun(lines, from: i) {
                    chunks.append(lines[i..<run.end].joined(separator: "\n"))
                    i = run.end
                    continue
                }
            }
    ...
    private static func parse(_ chunk: String, lang: AppLanguage, depth: Int) -> MDBlock {
        ...
        // The mirror of the same test in `split`. The two scanners must agree or the work is undone:
        // `split` groups a whole unfenced file into one chunk, and if this read it back as prose the
        // box that grouping existed for would never be drawn. `split` cuts the run at its last kept
        // line, so a chunk it produced always reaches its own end here.
        if let run = bareCodeRun(lines, from: 0), run.end >= lines.count {
            return .code(lang: run.lang, chunk)
        }
