    ...
    var id: String {
        ...
        case .table(let header, let rows):
            return "t\(header.count)x\(rows.count):"
                + MDBlock.digest(header.map { String($0.characters) }.joined(separator: "|"))
...
enum MarkdownBlocks {
    ...
    private static func parse(_ chunk: String, lang: AppLanguage, depth: Int) -> MDBlock {
        ...
        if lines.count >= 2, first.contains("|"), isTableDelimiter(lines[1]) {
            let header = tableCells(first).map { MarkdownInline.structured($0, lang: lang) }
            var rows: [[AttributedString]] = []
            var j = 2
            while j < lines.count {
                if isBlank(lines[j]) { j += 1; continue }
                rows.append(tableCells(lines[j]).map { MarkdownInline.structured($0, lang: lang) })
                j += 1
            }
