...
struct MathIslandStyle: Hashable, Sendable {
    ...
    var key: String {
        ...
        /* THE GROUND IS NOT PART OF THE IDENTITY ANY MORE. The page paints none, so two
           surfaces that differ only in what sits behind the equation now want the SAME
           bitmap — and while the ground was in this key they wanted two, so opening a
           conversation on a card missed every glyph it had already drawn on the page and
           the reader watched half the equations fall back to text. The ink stays: a light
           theme and a dark one genuinely need different bitmaps. */
        return textHex + "|" + errorHex + "|" + String(tenths)
    }
