...
private struct MarkdownStyle {
    ...
    /* THE COLOUR AN EQUATION IS BAKED AGAINST, and every row that touches the island has to use
       the SAME one or none of them agree. A KaTeX snapshot is opaque again and the ground is back
       in the cache key, so `MathIslandStyle` for `surface` and `MathIslandStyle` for `background`
       are two different keys for the same formula. The priming pass already knew the caller's
       ground; the rows below did not, and asked on `palette.background` — so on a card (a Brain
       answer, a mission, an artifact, an agent row) every primed glyph was missed, every equation
       was typeset a second time, and the bitmap that finally landed carried the PAGE's shade onto
       the card. One value, read by the pass that draws them and by the rows that collect them. */
    let ground: Color
