...
struct CompactDrawer: View {
    ...
    private var panel: some View {
        SidebarView(env: env)
            ...
            /* THE STRIP STARTS BELOW THE HEADER, and that is the same lesson `edgeSwipe` below
               records in full: a transparent view carrying a gesture is still the top-most thing in
               the hit test, so it silently eats every ordinary TAP in its band — a `DragGesture`
               with a minimum distance simply fails and the button underneath never hears the touch.
               That bug already cost this app its toolbar drawer button once, on the leading edge.

               On the trailing edge the casualty was the drawer's own close button: the header pads
               its controls 14 pt from the edge, so the last ~10 pt of that 44 pt target sat under
               this strip and a clean tap there did nothing. Clearing the header's own height gives
               the button all of itself back and costs the closing swipe only the one band where the
               close button already is.

               THE FOOT HAD THE SAME WOUND, and a worse one. `SidebarView.footer` pads to 12, and the
               last thing in `SidebarAccountPill.row` is `newChatButton` — which `SidebarView.body`
               calls out as the one and only new-conversation control in this panel. Inset 12 under a
               24 pt strip, the trailing 12 pt of that 40 pt disc was dead: a third of the drawer's
               most important target, and unlike a row it has nowhere else to be tapped. Clearing the
               header alone would have fixed the button the owner circled and left that one broken.

               Both bounds are read from the bottom-most and top-most CONTROL, not from the panel, so
               the strip keeps the whole history list — the tall middle, where a closing swipe
               actually starts. */
            .overlay(alignment: .trailing) {
                Color.clear
                    .frame(width: 24)
                    ...
                    .contentShape(Rectangle())
                    .gesture(drag)
                    .padding(.top, Self.headerReach)
                    .padding(.bottom, Self.footerReach)
    ...
    /// How far down the closing drag strip has to start so it stops covering the header's buttons.
    ///
    /// This is `SidebarView.header`'s own height and nothing more: 12 pt above the row, a 44 pt
    /// control, 4 pt below it. All three are fixed — the chips are framed, not typeset, so no text
    /// setting moves this number — but if that row is ever rebuilt, this follows it.
    private static let headerReach: CGFloat = 60

    /// And how far up it has to stop, for the same reason at the other end.
    ///
    /// Only the LAST row of `SidebarView.footer` is measured, not the footer: the usage line above
    /// it is not a control, the guest sign-up button is full-width and loses nothing that matters,
    /// and the footer's own height changes with both of them. The account pill is always last and
    /// always 40 pt — `SidebarAccountPill` frames its avatar and its new-chat disc at 40 — over the
    /// footer's 12 pt of bottom padding. A number that depended on the guest slot would be wrong
    /// for half the people using the app.
    private static let footerReach: CGFloat = 52
}
