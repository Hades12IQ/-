...
struct SidebarView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            SidebarHistoryList(env: env, query: query)
                /* The new-conversation pill FLOATS over the list rather than sitting above it, which
                   is where Claude puts it and where a thumb already is. It also stops the list from
                   being pushed down by a control that is used once a session. */
                .overlay(alignment: .bottom) { newConversationPill }
            footer
        }
    ...
    private var newConversationPill: some View {
        Button {
            Haptics.select()
            /* Animated on purpose. `newConversation` closes the drawer, and closing it without an
               animation snapped the panel away and let it flicker back — "يختفي السايد بار
               بعدها ترجع على الخفيف وهذا نشاز". One transition, one direction. */
            withAnimation(FirasMotion.sheet) {
