...
struct SidebarHistoryList: View {
    ...
    private var content: some View {
        let all = rows
        /* Pinned conversations are never trimmed away: someone pinned them precisely so they would
           be here. The budget applies to the rest. */
        ...
        if visible.isEmpty {
            ...
            list(buckets(of: visible), trimmed: visible.count < all.count)
        }
    }
