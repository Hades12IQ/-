...
struct AppShell: View {
    ...
    ///
    /// The opening swipe is not here either, and it is not a view: `CompactDrawer` installs a
    /// screen-edge recognizer on the window. Nothing in this stack covers the leading edge of the
    /// conversation, so a tap on the toolbar's own drawer button — or on anything else within
    /// 20 pt of the edge — still reaches it.
    @ViewBuilder
