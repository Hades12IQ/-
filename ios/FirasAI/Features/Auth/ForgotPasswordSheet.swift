...
struct ForgotPasswordSheet: View {
    ...
    private func perform() {
        ...
        case .forgot:
            ...
            Task {
                _ = await env.session.forgotPassword(email: address)
                // The endpoint answers 200 whether or not the address exists, and a rate limit is
                // the only real failure. Either way the honest sentence is the same one.
