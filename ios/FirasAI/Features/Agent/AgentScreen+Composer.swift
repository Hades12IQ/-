...
struct AgentComposer: View {
    ...
    private var controls: some View {
        HStack(spacing: 4) {
            ...
            ComposerActionButton(
                symbol: isBusy ? "stop.fill" : "arrow.up",
                label: isBusy ? Strings.Agent.stopMission(lang) : Strings.Agent.sendMission(lang),
                palette: palette,
                prominent: true,
                enabled: isBusy || (canSend && !isReading),
                action: { if isBusy { stopMission() } else { send() } }
            )
        }
    }
    ...
    /* THE MISSION THAT IS ACTUALLY RUNNING, which is not always this one: `isBusy` is true here
       whenever ANY mission is live, because the Agent runs one at a time. Stopping "this
       conversation" while the live mission belongs to another would report success and change
       nothing. */
    private func stopMission() {
        let target = env.agent.liveConversationID ?? conversationID
        Task { await env.agent.stop(in: target) }
    }
