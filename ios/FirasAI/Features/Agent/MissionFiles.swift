...
struct MissionFiles: View {
    ...
    private func documentRow(_ file: AgentFile) -> some View {
        Button {
            guard let request = request(for: file) else { return }
            Haptics.select()
            onOpen(request)
        } label: {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(file.name.isEmpty ? Strings.Agent.openFile(lang) : file.name)
                        .font(.system(size: 14))
                        .foregroundStyle(palette.textPrimary)
                        .lineLimit(2)
        ...
        .buttonStyle(.plain)
        .accessibilityLabel(Text(Strings.Agent.openFileNamed.fmt(lang, file.name)))
    }

    private func request(for file: AgentFile) -> MissionArtifactRequest? {
...
private struct MissionImageTile: View {
    ...
    var body: some View {
        Button {
            guard let index = file.artifactIndex else { return }
            onOpen(
    ...
    private func load() async {
        ...
        guard let index = file.artifactIndex else {
            failed = true
            return
        }
        let jobID = file.artifactJobID ?? job.id
