...
struct CodeAIBar: View {
    ...
    /// The 1 200-character ceiling is not applied mid-dictation: cutting the tail off a partial
    /// would move the text the recogniser is about to replace, and the next partial would be
    /// pasted after the stump instead of over it. The take is clamped once, when it lands.
    private func clamp(_ value: String) {
        guard !dictating, value.count > Self.instructionLimit else { return }
        draft = String(value.prefix(Self.instructionLimit))
    }

    // MARK: - Dictation

    /// One controller and one bar, shared with the chat and Agent composers. The field's own draft
    /// goes to the controller, so the words land here as they are spoken.
