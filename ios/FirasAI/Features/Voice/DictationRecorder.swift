...
final class DictationRecorder: Sendable {
    ...
    /// The two corrections a transcription engine actually notices, applied once to the finished
    /// take. Neither of them changes a word: they change how clearly the words arrive.
    ///
    /// 1. **A 70 Hz one-pole high-pass.** It removes the DC offset some inputs carry and the
    ///    handling rumble every phone held in a hand carries. Both sit far below speech, and both
    ///    eat headroom that step 2 would otherwise spend on noise.
    /// 2. **Gentle peak normalisation.** A phone held at arm's length in a quiet room records
    ///    speech at a fraction of full scale, and a quiet recording is the single most common
    ///    reason a transcript comes back with holes in it. The take is lifted so its loudest
    ///    sample sits at 0.82 of full scale — never louder, so it cannot clip — and never by more
    ///    than 4× (12 dB), so a silent room is not amplified into hiss. A take that is already
    ///    loud, or one with no signal in it at all, is returned untouched.
    ///
    /// Both passes are plain integer arithmetic over the buffer and run off the main thread.
    static func conditioned(_ pcm: Data) -> Data {
        // Under 10 ms there is nothing worth conditioning, and the floor in `finish()` will
        // reject the take anyway.
        guard pcm.count >= 320 else { return pcm }

        var out = pcm
        // A trailing half sample cannot be read as `Int16`; it is silence either way.
        if out.count % 2 == 1 { out.removeLast() }
        ...
        out.withUnsafeMutableBytes { rawBuffer in
            ...
            for index in 0..<samples.count {
                let input = Float(samples[index])
                let filtered = highPassCoefficient * (previousOutput + input - previousInput)
                previousInput = input
                previousOutput = filtered
                samples[index] = clamped(filtered)
                let magnitude = filtered < 0 ? -filtered : filtered
                if magnitude > peak { peak = magnitude }
            }

            let gain = gainForPeak(peak / 32_768)
            guard gain > 1.01 else { return }
            for index in 0..<samples.count {
                samples[index] = clamped(Float(samples[index]) * gain)
            }
        }
    ...
    /// `exp(-2π · 70 Hz / 16 000 Hz)`, written out so the filter cannot drift with the sample rate
    /// by accident: it is the coefficient for this recorder's 16 kHz output and nothing else.
    static let highPassCoefficient: Float = 0.9725

    /// How much to lift a take whose loudest sample is `peak` (0…1). Returns 1 — do nothing — for
    /// a take that is already loud enough and for one with no speech in it.
    static func gainForPeak(_ peak: Float) -> Float {
        guard peak > 0.03, peak < 0.5 else { return 1 }
        return min(0.82 / peak, 4)
    }

    private static func clamped(_ value: Float) -> Int16 {
...
private actor DictationRecorderCore {
    ...
    func start(feed: DictationAudioFeed?) async throws -> AsyncStream<Float> {
        ...
        // Two settings that only matter for transcription quality, and matter a lot.
        //
        // The hardware usually hands us 48 kHz; the recogniser wants 16. A cheap decimation folds
        // everything above 8 kHz back down on top of the speech as aliasing, and aliasing is heard
        // by a transcription engine as consonants that were never said — exactly the class of
        // mistake this take is trying to avoid. The best sample-rate converter the framework has
        // costs nothing here: it runs on 100 ms of mono audio at a time.
        //
        // `downmix` matters for the same reason on a stereo or multi-channel input (a connected
        // headset, an external interface): a real downmix instead of "keep channel 0" is the
        // difference between hearing the speaker and hearing whichever microphone faced away.
    ...
    func stop() async throws -> Data {
        ...
        self.sink = nil
        // Conditioning runs here, on this actor, so a five-minute take never touches the main
        // thread — and after `finish()`, so the level meter is never fed the corrected samples.
        return DictationRecorder.conditioned(data)
    }
