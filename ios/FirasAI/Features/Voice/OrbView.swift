...
private extension OrbView.Mode {
    ...
    ///
    /// `nonisolated` on both members below is deliberate: `OrbPainter` is a file-scope, un-isolated
    /// enum, and it reads `ringSpread` from a synchronous static function. Spelling the isolation
    /// out here means the painter compiles whatever an SDK decides about a type nested inside a
    /// `@MainActor`-inferred `View`.
    nonisolated var levelFloor: Double {
        switch self {
    ...
    nonisolated var ringSpread: CGFloat {
        switch self {
