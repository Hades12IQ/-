...
///
/// Whichever of the two produced the final text, it is then read once more by a model before it
/// settles: a four-second repair turn that fixes mis-hearings, puts Arabic words that came back in
/// Latin letters back into Arabic script, and punctuates — and changes nothing else. That pass
...
final class DictationController {
    ...
    /// True for the few seconds the repair turn is reading the transcript back. The words are
    /// already on screen while this is true — nothing is hidden behind it, it only says why the
    /// sentence may still change once.
    ...
    func finish() async -> String? {
        ...
        switch outcome {
        case .text(let heard):
            return await settle(heard, generation: generation)

        case .quiet:
            if !rescue.isEmpty { return await settle(rescue, generation: generation) }
            state = .idle
            isActive = false
            restoreDraft()
            clearTarget()
        ...
        case .failure(let message):
            ...
            // worse — and infinitely better than an empty composer. It is also the transcript that
            // most needs the repair pass, which is exactly what the owner asked for: the phone's
            // own result must not be the last word.
            if !rescue.isEmpty {
                toasts.show(Strings.Voice.micKeptDevice(prefs.lang), isError: false)
                return await settle(rescue, generation: generation)
            }
            restoreDraft()
            clearTarget()
            fail(message)
    ...
    // MARK: - Settling the take

    /// Puts the words on screen, gives the repair turn its four seconds, and settles whichever
    /// sentence survives.
    ///
    /// The order matters. A live take sees the raw transcript **first** — the moment it exists —
    /// and only then watches it be corrected. Nothing is ever held back waiting for the model: if
    /// the repair never lands, the reader has lost no time and no words.
    private func settle(_ heard: String, generation: Int) async -> String? {
        ...
        // The real transcript, on screen, before the repair turn is even asked for: in the draft
        // when a composer gave us one, and in the bar's own line when it did not.
        partial = raw
        if field != nil { writeLive(raw) }

        let repaired = await repair(raw)

        // Cancel (or a fresh take) while the repair was in flight: this sentence belongs to nobody.
        guard generation == takeGeneration else { return nil }
        return deliver(repaired)
    }

    /// One short model turn over the finished transcript, or the transcript itself.
    ///
    /// Offline, backgrounded, or a sentence too long to repair inside the budget — each of those
    /// returns the raw text without spending a request on it.
    private func repair(_ raw: String) async -> String {
        if skipPolishOnce {
            skipPolishOnce = false
            return raw
        }
        guard network.isOnline else { return raw }
        guard Self.isWorthRepairing(raw) else { return raw }

        let client = api
        let dialect = prefs.dictationDialect

        isPolishing = true
        let repaired = await Self.polished(raw, api: client, dialect: dialect)
        isPolishing = false
        return repaired
    }
    ...
    /// The one sentence that belongs on screen right now, whatever the take is doing — including
    /// the four seconds the repair turn spends reading the transcript back. A bar that shows this
    /// instead of a fixed string tells the truth at every step and never claims to be recording
    /// when it is not.
    func statusLine(_ lang: AppLanguage) -> String? {
        switch state {
        case .idle:
            return nil
        case .recording:
            return Strings.Voice.micListening(lang)
        case .transcribing:
            return isPolishing
                ? Strings.Voice.micPolishing(lang)
                : Strings.Voice.micTranscribing(lang)
