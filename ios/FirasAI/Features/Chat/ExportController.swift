    ...
    private func primeMath(_ sources: [String], palette: FirasPalette) async {
        ...
        outer: for source in sources {
            for chunk in ExportController.flowChunks(of: source) {
                guard let tex = ExportController.displayTex(chunk),
                      MathScanner.isTypesettable(tex) else { continue }
                let item = MathIslandItem(tex: tex, isDisplay: true)
                guard seen.insert(item.id).inserted else { continue }
                items.append(item)
                // The island's own queue stops at 240; asking for more is asking for nothing.
                if items.count >= ExportController.mathCeiling { break outer }
            }
        }
