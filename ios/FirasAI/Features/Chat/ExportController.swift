...
final class ExportController {
    ...
    func export(_ format: Format, markdown: String, title: String) async {
        _ = await produce(
            format,
            source: .document(markdown: markdown, template: nil, subtitle: ""),
            title: title,
            publish: true
        )
    }
    ...
    private enum Source {
        ...
        /* AND WHAT THE MODEL NAMED FOR IT. A `firas-file` block carries a `template` and a
           `subtitle`, chosen by the model for this particular document — an exam paper asks
           for the ministry look, a feasibility study for the corporate one. Resolving the
           template from the request text is the fallback for when it named none, not the
           first answer: the model knows what it wrote. */
        case document(markdown: String, template: DocTemplate?, subtitle: String)
    ...
    private func printedPDF(source: Source, title: String, lang: AppLanguage) async -> Data? {
        ...
        let subtitle: String
        switch source {
        case .document(let body, let named, let sub):
            markdown = body
            template = named
            subtitle = sub
        ...
        case .conversation(let conversation, let conversationLang):
            ...
            // A transcript wears no template: it is a record, not a designed document.
            template = nil
            subtitle = ""
    ...
    private func buildInput(for source: Source) -> ExportBuildInput? {
        switch source {
        case .document(let markdown, _, _):
