...
///
/// The panel is built from a value, not from the raw markdown, so it can be handed a NEW spec for
/// the same message id: while the answer streams, the tail of the JSON keeps arriving and the
/// parser's verdict changes from "one question" to "three". SwiftUI keeps `@State` across that
/// substitution, so the seeded answers are rebuilt — and the step index clamped — whenever the
/// question set itself changes. Without that, a wizard that grew mid-stream showed step 1 of 3 with
/// nothing pre-selected and a Confirm that did nothing.
struct AskPanelView: View {
    ...
    /// hands over a different set of questions or options.
    private var shape: String {
        var parts: [String] = []
        parts.reserveCapacity(spec.questions.count)
        for question in spec.questions {
            parts.append(question.id + ":" + question.options.map(\.label).joined(separator: "~"))
        }
    ...
    private var footer: some View {
        HStack(spacing: 10) {
            ...
            .buttonStyle(.plain)
            /* §7.7: Confirm is DISABLED when nothing is selected and the extra field is empty —
               not enabled-and-silent, which is defect D9. It stays enabled while the answer above
               is still streaming so the tap can say why (`onBlocked`) instead of doing nothing. */
            .disabled(answered || !primaryEnabled)
            .frame(minHeight: 44)
        }
    }
