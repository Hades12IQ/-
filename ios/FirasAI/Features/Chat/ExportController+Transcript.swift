...
///
/// **Math is flattened per turn, never across the document.** `MathScanner`'s `$$…$$` pairing is
/// unbounded, so a single unbalanced `$$` in turn 2 pairs with turn 8's and swallows six turns whole.
/// Flattening inside each turn makes that impossible by construction: a delimiter cannot reach past
/// the answer it was written in.
///
...
enum ExportTranscript {
    ...
    static func blocks(_ conversation: ChatConversation, lang: AppLanguage) -> [ExportBlock] {
        ...
        forEachTurn(conversation, lang: lang) { turn in
            ...
            out.append(contentsOf: ExportMarkdown.blocks(from: ExportText.flattenMath(turn.body)))
        }
        return out
    }
