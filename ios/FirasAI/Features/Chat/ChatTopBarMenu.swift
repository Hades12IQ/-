...
struct ChatTopBarMenu: View {
    ...
    /// Asks the screen for its format picker. The menu deliberately does not pick — nine formats
    /// with a sentence each need a sheet, and a sheet presented from a toolbar item is a sheet
    /// that dies with the toolbar. `ChatScreen` owns the one `.sheet` both halves travel through.
    ...
    /// menu opens it: `ShareSheetView` owns the link and the 20-share cap.
    ///
    /// A guest is turned back here rather than inside the sheet. `ShareController.createLink`
    /// refuses and routes to the sign-up prompt on its first `task`, so the old path opened a
    /// sheet that appeared and swapped itself out in the same breath. The per-message share
    /// (`ChatTurnActions.share`) has always asked the question before opening anything; this asks
    /// it in the same place, so both share entries behave the same way for the same account.
    ...
    ///
    /// ONE ROW, NOT A SUBMENU OF NINE. This was a nested `Menu` of nine file extensions, which asks
    /// the reader to already know which one they want — «مو مرتب نفس الموقع». `ExportFormatPicker`
    /// has been the finished answer to that for a while: three groups by what the file is *for*,
    /// and a sentence under every name. It needs the room a sheet has, so this row only asks for
    /// it; `ChatScreen` presents it and calls the same export that was here before.
    @ViewBuilder
