...
struct TranscriptView: View {
    ...
    var body: some View {
        ...
        return ScrollViewReader { proxy in
            ...
            /* NO `defaultScrollAnchor(.bottom)` HERE, AND THIS IS WHY.
               It does exactly what it says: when the content is SHORTER than the viewport it
               aligns that content to the bottom — so a conversation of one question sat at the
               foot of an empty screen with the whole page blank above it. That is the sinking
               bubble, and it was the price of using it to keep a long thread pinned.
               The settle passes in `seed` do the same job without that cost: they re-assert the
               bottom after the rows have finished changing size, and a short conversation is
               simply already at its bottom, so they do nothing at all. */
