...
enum ExportWeb {
    ...
    static func document(title: String, blocks: [ExportBlock]) -> String {
        ...
        // The transcript opens with its own `# title`; printing it again under the `<h1>` shows the
        // same words twice in a row. Word does the same dedupe, for the same reason.
        ...
        // `dir` only where a block disagrees with the page, so an English answer inside an Arabic
        // transcript reads left to right without every Arabic paragraph carrying an attribute.
        func mark(_ block: ExportBlock) -> String {
            let blockRTL = ExportOOXML.isRightToLeft(block, fallback: rtl)
            return blockRTL == rtl ? "" : (blockRTL ? " dir=\"rtl\"" : " dir=\"ltr\"")
        }
        ...
        for block in remaining {
            ...
            case .bullet(_, let spans):
                ...
                listOrdered = false
                listBuffer.append("<li" + mark(block) + ">" + inline(spans) + "</li>")
                continue
            ...
            case .heading(let level, let spans):
                ...
                body += "<" + tag + mark(block) + ">" + inline(spans) + "</" + tag + ">\n"
            case .paragraph(let spans):
                body += "<p" + mark(block) + ">" + inline(spans) + "</p>\n"
            case .quote(let spans):
                body += "<blockquote" + mark(block) + "><p>" + inline(spans) + "</p></blockquote>\n"
            ...
            case .math(let text, let tex):
                // Centred, left to right, in a mathematical face — and carrying its own source in
                // `data-tex`, so a reader who wants the LaTeX back can take it out of the page
                // without it being printed twice on the way past.
                body += "<div class=\"math\" dir=\"ltr\""
                if !tex.isEmpty {
                    body += " data-tex=\"" + ExportOOXML.escapeInline(tex) + "\""
                }
                body += ">" + ExportOOXML.escapeInline(text) + "</div>\n"
            case .table(let header, let rows):
                body += table(header: header, rows: rows, mark: mark(block))
            case .rule:
                body += "<hr>\n"
    ...
    private static func table(header: [String], rows: [[String]], mark: String) -> String {
        var html = "<table" + mark + ">\n"
        if !header.isEmpty {
            html += "<thead><tr>"
            for cell in header {
    ...
    *{box-sizing:border-box}
    ...
    .math{direction:ltr;text-align:center;overflow-x:auto;margin:1.5rem 0;font-size:1.15rem;\
    line-height:1.7;font-family:'Cambria Math','Latin Modern Math','STIX Two Math',Cambria,serif}
