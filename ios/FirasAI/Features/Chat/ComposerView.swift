...
struct ComposerView: View {
    ...
    private var primaryButton: some View {
        if isBusy {
            ComposerActionButton(
                symbol: product == .agent ? "hourglass" : "stop.fill",
                label: product == .agent ? Strings.Composer.agentBusy(lang) : Strings.Common.stop(lang),
                palette: palette,
                prominent: product != .agent,
                enabled: product != .agent,
                action: stop
            )
    ...
    private func send() {
        ...
        Haptics.send()
        FirasSound.shared.play(.send, prefs: env.prefs, callActive: env.call.isActive)
        pulse()
    ...
    private func stop() {
        guard product != .agent else {
            toast(Strings.Composer.agentBusy)
            return
        }
        Haptics.stop()
    ...
    private func startDictation() {
        ...
        dictating = true
        fieldFocused = false
        Task { await env.dictation.start() }
    }
