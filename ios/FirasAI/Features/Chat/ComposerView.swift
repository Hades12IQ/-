...
struct ComposerView: View {
    ...
    /* THROUGH `resolve`, NEVER THROUGH THE RAW SUBSCRIPT — the note `TranscriptView.liveState`
       carries, with a worse consequence here. `states` is keyed by the LOCAL id
       (`ChatStore.state(for:)` resolves before it files), so a screen opened by SERVER id — a
       notification tap, a shared link — found nothing: `isBusy` was false for the whole turn, so
       the composer showed Send where Stop belonged and the reader had no way to stop an answer
