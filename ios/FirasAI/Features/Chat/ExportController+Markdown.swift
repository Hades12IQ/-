...
enum ExportBlock: Sendable, Equatable {
    ...
    /// A display equation, on its own line. `text` is the readable form every writer prints;
    /// `tex` is the source it came from, carried so a page can offer it for copying.
    ///
    /// It is a block of its own and not a paragraph of symbols because that is the whole
    /// difference the owner asked for: a paragraph sets flush with the prose around it, an
    /// equation stands apart, centred, with air above and below.
    case math(text: String, tex: String)
    case rule
}
...
///
/// **Math is flattened here, per block, and nowhere upstream.** A whole document handed to
/// `MathScanner` at once lets one unbalanced `$$` on page two pair with page nine's and swallow
/// everything in between; a paragraph cannot reach past itself. A `$$…$$` that owns its line
/// never becomes prose at all — it comes out as `.math`, which is the only reason a writer can
/// set an equation apart from the sentence before it.
enum ExportMarkdown {

    // MARK: - Blocks

    /// Takes markdown as the model wrote it — LaTeX and all. Do not pre-flatten: this is the one
    /// place that decides which `$` is an equation and which is a price.
    static func blocks(from markdown: String) -> [ExportBlock] {
        ...
        func flush() {
            ...
            // A paragraph that is nothing but one equation is a display equation, whatever
            // delimiters it was written with. Setting it as prose is what makes ten integrals
            // read as one grey wall.
            if let lone = loneEquation(joined) {
                blocks.append(.math(text: symbols(lone), tex: lone))
                return
            }
            let spans = inlines(from: readable(joined))
        ...
        while index < lines.count {
            ...
            // Checked after the fence, so `$$` inside a code sample stays code, and before the
            // rule and heading tests, neither of which a `$` line can satisfy.
            if trimmed.hasPrefix("$$"), let display = displayMath(lines, from: index) {
                flush()
                if !display.tex.isEmpty {
                    blocks.append(.math(text: symbols(display.tex), tex: display.tex))
                }
                index = display.next
                continue
            }
            ...
            if let heading = headingLevel(trimmed) {
                flush()
                let text = readable(
                    String(trimmed.dropFirst(heading)).trimmingCharacters(in: .whitespaces)
                )
            ...
            if trimmed.hasPrefix(">") {
                ...
                let spans = inlines(from: readable(quoted.joined(separator: " ")))
                if !plain(spans).trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            ...
            if let item = listItem(raw) {
                flush()
                let spans = inlines(from: readable(item.text))
                if item.ordered {
    ...
    private static func cells(_ line: String) -> [String] {
        // The scanner runs on the whole row, not on each cell: a `|` inside `$…$` is part of the
        // equation (`\{x \mid x>0\}`), and splitting first would cut it in half.
        let (protected, spans) = MathScanner.protect(line)
        var working = Substring(protected.trimmingCharacters(in: .whitespaces))
        if working.first == "|" { working = working.dropFirst() }
        if working.last == "|" { working = working.dropLast() }
        return working
            .split(separator: "|", omittingEmptySubsequences: false)
            .map { cell -> String in
                let restored = MathScanner.restore(
                    String(cell).trimmingCharacters(in: .whitespaces),
                    spans: spans
                )
                return plain(inlines(from: readable(restored)))
            }
    }

    // MARK: - Math

    /// `$$…$$` opened at the start of a line, on one line or several. `nil` when the closing pair
    /// never arrives — a lone `$$` in prose is two dollar signs, not the start of an equation that
    /// eats the rest of the document.
    private static func displayMath(_ lines: [String], from index: Int) -> (tex: String, next: Int)? {
        ...
        while cursor < lines.count {
            ...
            if let close = line.range(of: "$$") {
                body += " "
                body += String(line[line.startIndex..<close.lowerBound])
                return (body.trimmingCharacters(in: .whitespacesAndNewlines), cursor + 1)
            }
            body += " "
            body += line
            cursor += 1
    ...
    /// The bare TeX of a line that is *only* an equation, whatever delimiters it used — `nil` for
    /// a line that also carries a word. Asked of the one scanner, so "the fee is $40" is never it.
    private static func loneEquation(_ text: String) -> String? {
        ...
        guard protected.trimmingCharacters(in: .whitespacesAndNewlines) == MathScanner.token(0) else {
            return nil
        }
    ...
    /// One block's *prose* with its equations turned into symbols a person can read. Bounded to
    /// the block it was called with, which is what stops a delimiter reaching into the next one.
    private static func readable(_ text: String) -> String {
        guard text.contains("$") || text.contains("\\") else { return text }
        return ExportText.flattenMath(text)
    }

    /// One **bare** expression — no delimiters — as symbols. `flattenMath` cannot be used here:
    /// it looks for delimiters to decide what is math, and a bare `\int_0^1 x\,dx` has none, so it
    /// would hand back the LaTeX untouched.
    private static func symbols(_ tex: String) -> String {
        let unicode = MathText.unicode(tex)
        return unicode.isEmpty ? tex : unicode
    }
