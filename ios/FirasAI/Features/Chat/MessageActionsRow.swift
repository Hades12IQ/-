...
/// The compact row under an assistant answer: copy, regenerate, listen, export, more.
///
/// Ghost buttons, 32 pt, `textMuted` until touched (`design-brief.md §7.7`). On a pointer device the
/// row sits at 55 % and comes to full strength on hover; on touch it is always fully visible, because
/// there is no hover to reveal it with. Copy says `تم النسخ` in place for 1.4 s — never a toast for
/// the happy path, always one for the failure.
struct MessageActionsRow: View {
    ...
    private func listenButton(palette: FirasPalette, lang: AppLanguage) -> some View {
        ...
        return ghost(
            ...
            tint: speaking ? palette.accent : palette.textSecondary,
            palette: palette
        ) {
    ...
    private func moreMenu(palette: FirasPalette, lang: AppLanguage) -> some View {
        Menu {
            ...
            ghostLabel(symbol: "ellipsis", tint: palette.textSecondary, palette: palette)
        }
        .menuOrder(.fixed)
    ...
    private func ghost(
        ...
        Button(action: action) {
            ghostLabel(symbol: symbol, tint: tint, palette: palette)
        }
        .buttonStyle(.plain)
        .accessibilityLabel(Text(title))
    }

    private func ghostLabel(symbol: String, tint: Color, palette: FirasPalette) -> some View {
        Image(systemName: symbol)
            ...
            .background {
                RoundedRectangle(cornerRadius: 7, style: .continuous)
                    .fill(palette.surfaceSunken.opacity(0.001))
            }
