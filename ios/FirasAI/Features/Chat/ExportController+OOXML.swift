...
enum ExportOOXML {
    ...
    /// One `ExportBlock`'s own direction. Code, equations and rules take the document's, always: an
    /// expression reads left to right in every language — `∫₀^π` is not mirrored in Arabic — a code
    /// sample likewise, and a rule has nothing to be a direction about.
