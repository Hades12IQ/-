...
extension LongFileViewer {
    ...
    private func loadOnce() async -> Pass {
        let manifest: LongFileManifest
        do {
            manifest = try await env.api.longFileManifest(jobID: jobID)
        } catch {
        ...
        while loadedParts < ready {
            ...
            var fetched: LongFilePart? = nil
            do {
                fetched = try await env.api.longFilePart(jobID: jobID, index: index)
            } catch {
                if error is CancellationError { return .finished }
                /* `404 part_not_ready` and `409 artifact_not_ready` on a PART are the manifest's
                   count running one step ahead of the bytes behind it — the worker publishes
                   `partsDone` before the part is readable. Treating them the way a missing
                   ARTIFACT is treated is what put «لم يعد هذا الملف موجودًا على الخادم» in front of
                   a reader whose file was being written perfectly well; three of those in a row
                   and the screen was an error for good. Stop asking for this part, keep every page
                   already verified, and come back on the next pass.
                   Once the manifest says `complete`, though, a part that is still not there is not
                   a race — it is gone, and waiting for it forever is the worse answer. */
                let status = (error as? APIError)?.status ?? 0
                guard (status == 404 || status == 409) && !manifest.complete else {
                    return status == 404
                        ? .failed(LongFileViewerCopy.missing(lang))
                        : .failed(errorMessage(for: error))
                }
            }
            guard let part = fetched else { break }
    ...
    /// What a failure to read the MANIFEST means. Unlike a part, a 404 here really is a file that
    /// is not there (`server-chat-jobs-chats.md §4.4`: `404 unknown_artifact`).
    @MainActor
    private func manifestFailure(_ error: Error) -> Pass {
        if error is CancellationError { return .finished }
        guard let apiError = error as? APIError else { return .failed(errorMessage(for: error)) }
        switch apiError.status ?? 0 {
        case 409:
            // `artifact_not_ready` — the ordinary answer while the first batch is still being
            // written. A wait, not a failure.
            return .waiting
        case 404:
            return .failed(LongFileViewerCopy.missing(lang))
        case 400:
            return .failed(LongFileViewerCopy.notALongFile(lang))
        default:
...
enum LongFileViewerCopy {
    ...
    /// `400 not_a_longfile` — the id belongs to a job that never wrote a durable artifact.
    static let notALongFile = LText(
        ar: "هذه المهمة لم تُنتج ملفًا يمكن فتحه هنا.",
        en: "This task did not produce a file that can be opened here."
    )
