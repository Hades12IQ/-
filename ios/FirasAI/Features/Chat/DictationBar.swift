...
///
/// The words arrive live, and where they are *shown* depends on the composer. A composer that
/// gave the controller its draft binding is already printing them itself, letter by letter, and
/// the bar stays quiet so the same sentence is not on screen twice; a composer that only asked for
/// a transcript at the end gets the live line here instead.
struct DictationBar: View {
    ...
    /// The last thing the recogniser heard, replaced wholesale every time it refines itself.
    private var liveText: some View {
        Text(verbatim: dictation.partial)
            .font(.system(size: 14))
            .foregroundStyle(palette.textSecondary)
            .lineLimit(2)
            .frame(maxWidth: .infinity, alignment: .leading)
            .bidiIsland(for: dictation.partial, fallback: lang)
            .padding(.horizontal, 4)
            .accessibilityLabel(Text(Strings.Voice.micLiveText(lang)))
            .accessibilityValue(Text(verbatim: dictation.partial))
    }
    ...
    /// Only when the composer is not already showing the same words in its own field.
    private var showsLiveText: Bool {
        !dictation.isWritingLive && !dictation.partial.isEmpty
    }
