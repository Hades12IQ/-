...
/// share sheet for a finished export: it is not a destination, it carries a temp file that dies with
/// this screen, and routing it would mean teaching the router about `URL`s it can never restore.
struct ChatScreen: View {
    ...
    /// The whole conversation as one document. Nothing offered it before, so
    /// `ExportController.export(_:conversation:)` and the nine writers behind it were unreachable.
    @State private var transcriptExport: ExportController.Export?
    ...
    var body: some View {
        ...
        return content
            ...
            .sheet(item: $transcriptExport) { export in
    ...
    /// The web's chat-export button, gated on the same rule: a thread of nothing but questions is
    /// not a transcript, so there is nothing to hand anyone. Hidden entirely rather than disabled —
    /// a permanently grey button on a new conversation is noise.
    @ViewBuilder
    private var exportButton: some View {
        if let conversation, ExportTranscript.isExportable(conversation) {
            Menu {
                ForEach(ExportController.Format.allCases) { format in
                    Button {
                        exportTranscript(format, conversation: conversation)
                    } label: {
                        Text(format.label(env.prefs.lang))
                    }
                }
            } label: {
                Image(systemName: isExportingTranscript ? "hourglass" : "square.and.arrow.up")
            }
            .menuOrder(.fixed)
            .disabled(isExportingTranscript)
            .accessibilityLabel(Text(Strings.Chat.download(env.prefs.lang)))
        }
    }

    private func exportTranscript(
        _ format: ExportController.Format,
        ...
        Task {
            ...
            if let finished = controller.result { transcriptExport = finished }
        }
    }
