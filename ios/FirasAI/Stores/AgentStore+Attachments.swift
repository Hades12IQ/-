...
extension AgentStore {
    ...
    /* WHAT THE LAST MISSION BUILT, not only what it said about it. Without this a
       follow-up instruction began unaware that any deliverable existed, so «change the
       header» after a mission that built a site produced a second site from nothing. */
    private static let previousFilesHeader =
        "\n\n=== ملفات أنتجتها المهمة السابقة (اعمل عليها، لا تبدأ من الصفر) / FILES THE PREVIOUS MISSION PRODUCED (revise these, do not start over) ===\n"
    private static let previousFilesLimit = 1_400
    ...
    func foldedTask(
        ...
        /* NAMES AND TYPES, NEVER URLS. Every one of these carries a signed link to the
           owner's own host; a prompt sent to a third-party agent is not a place to put
           one, and the name is what the instruction will refer to anyway. */
        let manifest = Self.manifest(of: previousFiles)
        if !manifest.isEmpty {
            task += Self.previousFilesHeader + String(manifest.prefix(Self.previousFilesLimit))
        }

        return String(task.prefix(Self.taskLimit))
    }

    /// One line per deliverable: its name, and its kind when the name does not say.
    private static func manifest(of files: [AgentFile]) -> String {
        var lines: [String] = []
        var seen = Set<String>()
        for file in files.prefix(40) {
            let name = file.name.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !name.isEmpty, seen.insert(name).inserted else { continue }
            let kind = file.type.trimmingCharacters(in: .whitespacesAndNewlines)
            lines.append(kind.isEmpty ? "- " + name : "- " + name + " (" + kind + ")")
        }
