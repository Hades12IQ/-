...
extension MediaStore {
    ...
    func createMusic(prompt: String, lyrics: String?, seconds: Int, in conversationID: String?,
        ...
        /* THE READER'S OWN WORDS AS THE TITLE, while the song is being written. The card no
           ...
           reader's sentence anyway — but once the plan lands it becomes English production
           tags, and a card that showed those would be exactly the machine text the owner
           keeps finding on his screen. */
        await placeCard(
            kind: .music,
        ...
        let meta = MediaMeta(
            ...
            lyrics: plan.lyrics,
            title: plan.title,
            // Kept in the fence for the web, which shows the genre; the app's card deliberately
            // does not draw it — see `SongCard`.
            style: plan.style
        )
    ...
    func regenerate(_ creationID: String) async {
        ...
        case .music:
            ...
            let meta = MediaMeta(
                ...
                // Carried through the re-roll: a fence that loses it is a song the web can no
                // longer label.
                style: style
            )
    ...
    private func placeCard(
        ...
        upsert(
            MediaCreation(
                ...
                /* THE TURN IT LIVES IN, from the first frame. The fence written below carries no
                   cache key until the render lands, so nothing else can tell the library that this
                   creation and that placeholder are one thing — and a rebuild while the render was
                   still running used to file the fence a SECOND time, as a permanently timed-out
                   tile sitting beside the live one. */
                messageID: ChatMessage.identity(role: .assistant, cid: cid),
                createdAt: Date(),
                phase: .queued
            )
        )
    ...
    private func start(
        ...
        let draftCreation = MediaCreation(
            ...
            meta: meta,
            conversationID: target.local,
            // Same reason as `placeCard`: the turn is this creation's other identity, and it is the
            // only one that exists until the key does.
            messageID: ChatMessage.identity(role: .assistant, cid: cid),
            createdAt: Date(),
            phase: .queued
        )
