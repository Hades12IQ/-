...
final class CodeStore: JobObserver {
    ...
    @ObservationIgnored private var liveBuilds: [String: Task<Void, Never>] = [:]
    ...
    func open(_ id: String) async {
        ...
        if liveBuilds[id] == nil,
           let cached = await cache.load(id: id),
           liveBuilds[id] == nil {
    ...
    /// Starts a build. It runs **here**, in front of the reader, and is written to disk as a ticket
    /// first so that every way out of this screen still ends with the turn on the server.
    ///
    /// Nothing about this method awaits the build: the ticket write is the only suspension, and the
    /// builder itself is a task, so the composer gets its screen back immediately.
    ...
    private func runLiveBuild(_ ticket: CodeBuildTicket) async {
        ...
        do {
            ...
            steps = Self.parsePlan(raw, kind: kind)
        } catch {
        ...
        for step in steps {
            ...
            await cache.save(CodeProject(name: ticket.name, files: built), id: projectID)
        }
    ...
    /// screen: a build the reader is not looking at has nothing to paint, and `create` is the only
    /// caller that can start one, so this is always the project they are watching.
    ...
    func handOffLiveBuilds() {
        for ticket in Array(tickets.values) where liveBuilds[ticket.projectID] != nil {
            handOff(ticket)
        }
    }
