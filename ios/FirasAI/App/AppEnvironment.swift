...
final class AppEnvironment {
    ...
    private func wireMediaHandoff() {
        chat.onMediaRequest = { [weak self] kind, prompt, conversationID in
            ...
            switch kind {
            case .image:
                await self.media.createImage(prompt: prompt, shape: nil, in: conversationID)
            case .video:
            ...
            case .music:
                await self.media.createMusic(prompt: prompt, lyrics: nil, seconds: 60, in: conversationID)
            }
            return true
        }
    }
