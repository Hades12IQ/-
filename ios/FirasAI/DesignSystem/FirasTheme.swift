...
enum FirasTheme: String, CaseIterable, Codable, Identifiable, Sendable {
    ...
    /* THE PALETTE CARRIES THE THEME IT CAME FROM. Rows are drawn through `.equatable()`, whose
       `==` must be able to answer "is this the same paint?" without comparing forty colours.
       Without an identity it could not answer at all, so it did not ask — and a theme change
       left every conversation already on screen wearing the previous theme's ink. */
    ...
    private var built: FirasPalette {
        switch self {
        case .light:
            ...
            FirasPalette(
                ...
                grainOpacity: 0.022, haloOpacity: 0.030, isLightFamily: true
            )
        case .dark:
            ...
            FirasPalette(
                ...
                borderStrong: "45443E", accent: "7E9B93", accentHover: "8FAAA2",
                accentDeep: "56706A", onAccent: "1B1A19", success: "7BA894", error: "CE8A80",
                accentSoftAlpha: 0.05, accentRingAlpha: 0.24, glassTintAlpha: 0.018,
                ...
                grainOpacity: 0.030, haloOpacity: 0.032, isLightFamily: false
            )
        case .black:
            ...
            FirasPalette(
                ...
                borderStrong: "2E2E2C", accent: "83A099", accentHover: "94AEA7",
                accentDeep: "5B7570", onAccent: "000000", success: "7BA894", error: "CE8A80",
                accentSoftAlpha: 0.05, accentRingAlpha: 0.24, glassTintAlpha: 0.018,
                ...
                grainOpacity: 0, haloOpacity: 0.022, isLightFamily: false
            )
        case .midnight:
            ...
            FirasPalette(
                ...
                borderStrong: "303B52", accent: "8899A9", accentHover: "9AA8B6",
                accentDeep: "5A6B7B", onAccent: "0A0E19", success: "7BA894", error: "CE8A80",
                accentSoftAlpha: 0.05, accentRingAlpha: 0.24, glassTintAlpha: 0.018,
                ...
                grainOpacity: 0.026, haloOpacity: 0.032, isLightFamily: false
            )
        case .graphite:
            ...
            FirasPalette(
                ...
                borderStrong: "38383C", accent: "82969E", accentHover: "93A4AB",
                accentDeep: "586870", onAccent: "101012", success: "7BA894", error: "CE8A80",
                accentSoftAlpha: 0.05, accentRingAlpha: 0.24, glassTintAlpha: 0.018,
                washAlpha: 0.035, strokeAlpha: 0.09, shadowAlpha: 0.16,
                userFill: "2B2B2F", maxTierText: "9E8ECB", planGold: "BFA06A", planDiamond: "8AA6C4",
                callGround: "171719", codeWarn: "C6A45F", codeOk: "7BA894",
                grainOpacity: 0.022, haloOpacity: 0.030, isLightFamily: false
            )
        case .amber:
            ...
            FirasPalette(
                ...
                borderStrong: "40382E", accent: "B0A08B", accentHover: "BFB09C",
                accentDeep: "7E7160", onAccent: "1B1713", success: "8FA87C", error: "CE8A80",
                accentSoftAlpha: 0.05, accentRingAlpha: 0.24, glassTintAlpha: 0.017,
                ...
                grainOpacity: 0.030, haloOpacity: 0.030, isLightFamily: false
            )
        }
    }
}
...
struct FirasPalette: Sendable {
    ...
    /// Which theme made this palette — `FirasTheme.rawValue`, stamped by `FirasTheme.palette`.
    /// It exists so an `Equatable` view can notice a theme change in one string comparison
    /// instead of forty colour comparisons it has no way to make.
    var id: String = ""
    ...
    // MARK: - The bubble's lean
    //
    // «كل ثيم يتغير بي لون فقاعته» — the reader's chip leans toward the theme's own
    // hue: green under the three green themes, blue under نيلي, amber under عنبري.
    //
    // The accent cannot be mixed in as it stands: it was drained on purpose («قلل النيون») and
    // mixing a near-grey into a near-grey moves nothing. So only its HUE is taken, given back a
    // working saturation, and folded into the bubble at 16-18% — enough to read as a lean, far
    // short of a coloured bubble, and derived per theme rather than picked by hand six times.

    private static func hexParts(_ hex: String) -> (Double, Double, Double) {
    ...
    /// The hue in degrees, or `nil` for a colour too close to grey to have one — in which case
    /// the bubble keeps the neutral it was given rather than inventing a red.
    private static func hue(_ parts: (Double, Double, Double)) -> Double? {
        let high = max(parts.0, parts.1, parts.2)
        let low = min(parts.0, parts.1, parts.2)
        let delta = high - low
        guard delta > 0.02 else { return nil }
        var h: Double
        if high == parts.0 {
            h = (parts.1 - parts.2) / delta
        } else if high == parts.1 {
            h = 2 + (parts.2 - parts.0) / delta
        } else {
            h = 4 + (parts.0 - parts.1) / delta
        }
        h *= 60
        return h < 0 ? h + 360 : h
    }

    private static func fromHSV(_ h: Double, _ s: Double, _ v: Double) -> (Double, Double, Double) {
        let c = v * s
        let sector = h / 60
    ...
    private static func leaning(_ base: String, toward accent: String, isLightFamily: Bool) -> Color {
        let ground = hexParts(base)
        guard let h = hue(hexParts(accent)) else { return Color(hex: base) }
        let tint = fromHSV(h, isLightFamily ? 0.45 : 0.50, isLightFamily ? 0.62 : 0.55)
        let amount = isLightFamily ? 0.16 : 0.18
        return Color(
            .sRGB,
            red: ground.0 + (tint.0 - ground.0) * amount,
            green: ground.1 + (tint.1 - ground.1) * amount,
            blue: ground.2 + (tint.2 - ground.2) * amount,
            opacity: 1
        )
    }
    ...
    init(
        ...
        self.userFill = FirasPalette.leaning(userFill, toward: accent, isLightFamily: isLightFamily)
