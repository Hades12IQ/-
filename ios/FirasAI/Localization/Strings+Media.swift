...
extension Strings {
    enum Media {
        ...
        /// The song card's save. A song is the one kind Photos refuses, so keeping one means the
        /// share sheet and, from there, Files — the app's answer to the web's `<a download>`
        /// (`app.js:4684-4689`). «حفظ الاغنية ماكو» was the report; this is the label on the fix,
        /// and it says *save* rather than «تحميل» because on a phone that is what it does.
        static let saveSong = LText(ar: "حفظ الأغنية", en: "Save the song")
        ...
        /// Never «ما ضبط التلحين»: the song was composed and is safe on the server; its file is
        /// what did not reach this device, and the way out is a free retry rather than a re-render.
        static let songDownloadFailed = LText(
            ar: "تعذّر تنزيل الأغنية. الأغنية محفوظة على الخادم — تحقّق من اتّصالك ثم أعد المحاولة.",
            en: "The song could not be downloaded. It is safe on the server — check your connection and try again."
        )

        static let preparingFile = LText(ar: "يجهّز الملف…", en: "Preparing the file…")

        // MARK: - Fetching the finished bytes

        /// The cover's word while a **finished** render comes down the wire. The loader words above
        /// describe making the thing and would be a lie here: the picture, the clip and the song all
        /// already exist on the server by the time these are on screen.
        static let fetchingImage = LText(ar: "أنزّل الصورة…", en: "Downloading the picture…")
        static let fetchingClip = LText(ar: "أنزّل المقطع…", en: "Downloading the clip…")
        static let fetchingSong = LText(ar: "أجهّز الأغنية…", en: "Getting the song ready…")
