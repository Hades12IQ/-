...
extension Strings {
    enum Settings {
        ...
        /// The sixth row. Not a `SettingsSection` — that enum is a frozen route list — and not a
        /// web tab either: the web has no privacy page yet. New copy.
        static let tabPrivacy = LText(ar: "الخصوصية", en: "Privacy")
        ...
        static let tabPrivacySub = LText(ar: "بياناتك وما يُستخدم منها", en: "Your data and what is used")
        ...
        enum Voice {
            ...
            // MARK: - The world language list (`DialectPickerSheet`)

            /// The two sections of the picker. "Suggested" is the fourteen the transcriber has a
            /// real hint for; "All languages" is everything `Locale` knows about, named in both
            /// scripts, because a person who speaks Japanese should not have to find themselves
            /// in a list of Arabic dialects.
            static let languageSuggested = LText(ar: "المقترحة لك", en: "Suggested for you")
            static let languageAll = LText(ar: "كل لغات العالم", en: "All world languages")
            static let languageSearch = LText(ar: "ابحث عن لغة", en: "Search for a language")

            /// The honest note under the world section: outside the fourteen there is no dialect
            /// hint to send, and automatic detection is what actually runs.
            static let languageAutoNote = LText(
                ar: "اللغات خارج القائمة المقترحة تُكتب بالتعرّف التلقائي — يعمل معها بلا تلميح للّهجة.",
                en: "Languages outside the suggested list are handled by automatic detection — it works, just without a dialect hint."
            )

            /// Footer count of the world list — the six Arabic forms of `لغة`, because "١٤٣ لغة"
            /// and "٣ لغات" are different words and a `{n}` replace would get one of them wrong.
            static let languageCountZero = LText(ar: "لا لغات", en: "No languages")
            static let languageCountOne = LText(ar: "لغة واحدة", en: "%ld language")
            static let languageCountTwo = LText(ar: "لغتان", en: "%ld languages")
            static let languageCountFew = LText(ar: "%ld لغات", en: "%ld languages")
            static let languageCountMany = LText(ar: "%ld لغة", en: "%ld languages")
            static let languageCountOther = LText(ar: "%ld لغة", en: "%ld languages")

            static func languageCount(_ n: Int, _ lang: AppLanguage) -> String {
                ArabicPlurals.count(
                    n,
                    lang,
                    zero: languageCountZero,
                    one: languageCountOne,
                    two: languageCountTwo,
                    few: languageCountFew,
                    many: languageCountMany,
                    other: languageCountOther
                )
            }
        ...
        enum Notifications {
            ...
            /// Provisional / ephemeral: iOS delivers quietly to the notification centre. The app
            /// never asks for it, but a device can arrive in that state, so it has its own copy
            /// instead of being lumped in with "On".
            static let statusQuiet = LText(ar: "صامتة", en: "Quiet")
            static let quietNote = LText(
                ar: "التنبيهات تصل بهدوء إلى مركز الإشعارات فقط. اسمح بها لتظهر على الشاشة.",
                en: "Notifications arrive quietly in Notification Centre only. Allow them to appear on screen."
            )

            /// The promise about *when* the app asks — the answer to "why am I being asked now?".
            static let whenNote = LText(
                ar: "لا يطلب التطبيق الإذن عند فتحه — يطلبه أول مرة تبدأ فيها مهمة طويلة، ومرة واحدة فقط.",
                en: "The app never asks when it opens — it asks the first time you start a long task, and only once."
            )

            /// What a finished-job notification actually contains, so the page describes the thing
            /// the user will see.
            static let sampleNote = LText(
                ar: "كل تنبيه يحمل اسم المنتج، اسم المحادثة، وسطرًا يقول ما الذي انتهى.",
                en: "Every notification carries the product, the conversation's name, and one line saying what finished."
            )

            /// What the lock screen shows in place of the real lines when previews are hidden.
            static let hiddenPreviewJob = LText(ar: "نتيجة مهمة جاهزة", en: "A task result is ready")
            static let hiddenPreviewCall = LText(ar: "انتهت مكالمة", en: "A call has ended")

            /// Shown after the system prompt was answered with "Don't Allow" in this session.
            static let justDenied = LText(
                ar: "تم رفض الإذن. يمكنك تفعيله من إعدادات النظام في أي وقت.",
                en: "Permission was declined. You can turn it on from system settings at any time."
            )
        }

        // MARK: - Privacy

        /// The one data question the app asks, in the two places it is asked: the first-run door
        /// (`ConsentView`) and the Privacy page (`PrivacySettingsView`). All new copy — the web
        /// has no equivalent screen — written to the same rule as the rest of the app: say what
        /// is true, say it in one sentence, and never dress a refusal as a smaller button.
        ///
        /// Nothing here promises anything happens off the device: today the answer is recorded
        /// locally, because the server exposes no preferences or consent route.
        enum Privacy {

            // MARK: The page

            static let trainingHeader = LText(ar: "تحسين فِراس", en: "Improving Firas")
            static let trainingToggle = LText(
                ar: "استخدام محادثاتي في تدريب النماذج",
                en: "Use my conversations to train the models"
            )
            static let trainingToggleHint = LText(
                ar: "لمساعدة فريق فِراس AI على تحسين جودة الردود",
                en: "helps the Firas AI team improve reply quality"
            )

            /// The sentence that describes the state the switch is in — the answer to "so what
            /// does this actually do?", which a hint under a switch never quite gives.
            static let meaningOn = LText(
                ar: "الآن: يمكن لفريق فِراس AI الاطّلاع على محادثاتك واستخدامها في تدريب النماذج وتحسينها.",
                en: "Right now: the Firas AI team may read your conversations and use them to train and improve the models."
            )
            static let meaningOff = LText(
                ar: "الآن: محادثاتك خارج التدريب، ولا يتغيّر شيء فيما يقدّمه لك فِراس.",
                en: "Right now: your conversations stay out of training, and nothing about what Firas gives you changes."
            )

            static let deviceNote = LText(
                ar: "يُحفَظ اختيارك على هذا الجهاز، ويمكنك تغييره في أي وقت.",
                en: "Your choice is kept on this device, and you can change it at any time."
            )
            static let notAnsweredNote = LText(
                ar: "لم يُسجَّل جواب على هذا الجهاز بعد، والوضع الحالي هو الرفض.",
                en: "No answer has been recorded on this device yet, so the current state is Decline."
            )
            static let saved = LText(ar: "تم حفظ اختيارك ✓", en: "Your choice is saved ✓")

            static let elsewhereHeader = LText(ar: "خصوصيتك", en: "Your privacy")

            // MARK: The first-run question

            static let consentTitle = LText(ar: "مساعدة فِراس على التحسّن", en: "Help Firas improve")
            static let consentQuestion = LText(
                ar: "هل تسمح لفريق فِراس AI باستخدام محادثاتك في تدريب النماذج وتحسينها؟ جوابك لا يغيّر أي ميزة في التطبيق.",
                en: "May the Firas AI team use your conversations to train and improve the models? Your answer changes no feature in the app."
            )

            static let acceptTitle = LText(ar: "أوافق", en: "Accept")
            static let acceptBody = LText(
                ar: "يمكن لفريق فِراس AI الاطّلاع على محادثاتي واستخدامها في تدريب النماذج.",
                en: "The Firas AI team may read my conversations and use them to train the models."
            )
            static let declineTitle = LText(ar: "لا أوافق", en: "Decline")
            static let declineBody = LText(
                ar: "تبقى محادثاتي خارج التدريب، ويعمل فِراس معي كما هو.",
                en: "My conversations stay out of training, and Firas works exactly as before."
            )

            static let changeLater = LText(
                ar: "يمكنك تغيير هذا لاحقًا من الإعدادات، قسم الخصوصية.",
                en: "You can change this later in Settings, under Privacy."
            )
            static let consentPending = LText(
                ar: "اختر «أوافق» أو «لا أوافق» في سؤال التحسين أعلاه.",
                en: "Choose “Accept” or “Decline” in the question above."
            )
        }
