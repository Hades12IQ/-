...
enum AppLanguage: String, CaseIterable, Codable, Sendable, Identifiable {
    ...
    /// ARABIC, WHATEVER THE PHONE SAYS.
    ///
    /// The web reads the device's preferred language and hands an English phone an English UI.
    /// That rule is wrong for THIS product: Firas AI is an Arabic-first assistant for students in
    /// Iraq, and a phone set to English is common there without its owner wanting an English app.
    /// The owner saw it himself — an Arabic conversation under English error text and an English
    /// toast — and it reads as a half-translated product rather than a preference.
    ///
    /// English is one switch away in Settings, and a device that has chosen it keeps it: this is
    /// only the value used before anyone has chosen.
    static var deviceDefault: AppLanguage { .arabic }
}
