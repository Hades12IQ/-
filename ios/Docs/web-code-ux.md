...
1. A **project is a chat** with `codeProj: true`. `messages[0]` is an assistant message whose whole content is a ```` ```firas-project ```` fence around `{ name, files:[{path,content}] }`; `messages[1]` is the Firas Code chat thread, a ```` ```firas-code-chat ```` fence around **base64(JSON)**. There is no other server-side representation (`app.js:60119-60127`, `60566-60606`, `60609-60657`).
...
All four require the member cookie (401 `{ error: "not authenticated" }`); an id that is not the caller's → `404 { error: "not found" }` on GET/PUT/DELETE; create/update bodies are read with a 2 000 000-byte cap and a non-JSON body → `400 { error: "invalid JSON body" }`; the server slices `title` to 200 chars (the 80-char cap is client-side) (`server.mjs:2549-2625`). Guests never call them: a guest's projects live only in device storage (`app.js:3358-3367`, `3461`).
...
Exact serialisation written by `codeSaveFiles` (`app.js:60566-60606`):
...
| `firas_code_pal_recent_v1` (`CW_PAL_REC_LS`) | command-palette recents, max 4, keyed by stable file paths (never indices) | `app.js:76548-76552` |
...
- **Blank**: name = typed name, else `titleFrom(description)` if a description exists, else `"مشروع جديد"` / `"new-project"`; creates the project with `CW_BLANK_FILES` (one `index.html`, `app.js:61128`) and opens the IDE. The scaffold is always Arabic/RTL regardless of UI language — reproduce it byte for byte:
  ...
  ```
  (the stored string ends with a trailing `\n`).
...
`jobId = sha1(ownerId).hex.slice(0,10) + "-" + jobKey(cid)` where `cid` is first stripped to `[A-Za-z0-9_-]` and cut at 64 (empty → `"j" + 12 hex chars`), and `jobKey` replaces `[.$#[\]/\s]` with `_` and cuts at 96 (`server.mjs:731-733`, `12571-12572`). Treat it as opaque; pass it back verbatim. The request body is read with `CHAT_BODY_LIMIT` = 25 000 000 bytes (`server.mjs:442`) before the 600 000-char queue check. For a **guest** the stored `chatId` is always `""` whatever was sent (`user ? chatId : ""`, `server.mjs:12626`). The stored record has `kind: "codebuild"`, `product: "code"`, `tier` (`TIERS[tier] ? tier : "pro"` — it is not what builds the code; the builder uses `CODEBUILD_TIER`), `lang`, `task` (≤8000), `name` is NOT stored on the record — it travels in the input payload and is read by the worker (`server.mjs:12626-12647`, `11488`).
...
- No record → `{ "phase": "unknown" }` (also after `JOB_KEEP_MS` 6 h). The query id goes through `jobKey` first, so an id containing `.`, `/`, `$`, `#`, brackets or whitespace is rewritten and reads as unknown — send the id exactly as received.
- While `queued` (every kind except `longfile`) the output record is not read: `text` is `""` (`server.mjs:12712-12717`).
...
loop every 4000 ms while (now - start) < 2h:
  ...
  if fetch threw, or the status is non-OK and not 403 (401, 5xx): j = null; continue   // a dropped poll is not a failure
...
| `product` not `code`/`agent` | `400 { error: "invalid product" }` (body read with a 2 000-byte cap) |
...
- File row: colour badge from `CW_FILE_BADGE` (`app.js:73440-73458`, keyed by lower-cased extension): html/htm `#e34c26 "<>"`, css `#8b5cf6 "#"`, js/mjs `#f7df1e "JS"`, jsx `#f7df1e "JX"`, ts `#3178c6 "TS"`, tsx `#3178c6 "TX"`, json `#a8a8a8 "{}"`, md `#519aba "M"`, py `#3572a5 "Py"`, svg `#ffb13b "◇"`, txt `#9aa0a6 "·"`, anything else `#9aa0a6 "·"`; basename only (full path as tooltip), size `max(1, round(chars/1024)) + "K"`, and a `✕` (`delF` `"حذف"`/`"delete"`) that asks `delFileC` `"حذف الملف؟"`/`"Delete this file?"`; deleting selects file 0 and refreshes the preview.
...
6. The document is then passed through `cwComputerInject(html)`: `CW_CONSOLE_HOOK + CW_COMPUTER_HOOK` is inserted **immediately after the opening `<head …>` tag** (regex `/<head([^>]*)>/i`), or prepended to the whole string when there is no `<head>` (`app.js:60809-60815`), and the result is set as `srcdoc`. Because the hook runs before any author script, every `console.*` call and every early throw is captured.
...
```json
// console.log / warn / error / info (the original console method still runs afterwards):
{ "__fcw": 1, "t": "log"|"warn"|"error"|"info", "m": "<args joined by a space; objects JSON.stringify(x).slice(0,300), others String(x); whole line slice(0,600)>" }
// window "error" event:
{ "__fcw": 1, "t": "error", "m": "<e.message> @ line <e.lineno>", "file": "<e.filename or ''>", "line": <e.lineno or 0>, "col": <e.colno or 0>, "stack": "<e.error.stack slice(0,1200) or ''>" }
// window "unhandledrejection" event (r = e.reason):
{ "__fcw": 1, "t": "error", "m": "Promise: <r.message or r>", "file": "<r.fileName or ''>", "line": <r.lineNumber or 0>, "col": <r.columnNumber or 0>, "stack": "<r.stack slice(0,1200) or ''>" }
```

The `" @ line N"` suffix on `window.error` messages is what the Problems panel warns about with `approx` (the line belongs to the merged preview document, not to the user's file).
...
`shareActiveChat` (`app.js:79686-79713`) behind the bar's Share button (which commits the editor first, `app.js:75627-75639`): guests → sign-up prompt; a chat with no messages → toast `"افتح محادثة فيها رسائل أولًا"`/`"Open a chat with messages first"`; `persistChat` first (a project gets its `serverId` here); `POST /api/share { chatId: serverId }` → `{ ok: true, id }`; link `origin + "/?share=" + id`; copy to clipboard and open the share sheet. Toasts: `"ينشئ رابط المشاركة…"`/`"Creating share link…"`, `"تم نسخ رابط المشاركة ✓"`/`"Share link copied ✓"`, failure `"تعذّر إنشاء الرابط — تأكد من تسجيل الدخول واتصالك ثم أعد المحاولة"`/`"Couldn't create the link — check you're signed in and online, then retry"`. Server (`server.mjs:9217-9262`): member cookie required (`401 { error:"auth required" }`), rate limit 5/min (`429 { error:"too many requests" }`), non-JSON body `400 { error:"invalid JSON" }`, unknown chat `404 { error:"not found" }`, re-sharing the same chat returns the same id, account cap `SHARES_PER_USER_MAX` = 20 links → `409 { error: "لقد وصلت إلى الحد الأقصى للمشاركات (20). احذف مشاركة قديمة أولاً." }` (Arabic in both UI languages, `server.mjs:9266-9269`); the snapshot keeps at most 400 messages, each `content` cut at 200 000 chars, so a very large project is truncated in the public copy (`server.mjs:9271-9272`). `GET /api/share?id=` strips the id to `[a-zA-Z0-9]` and needs no cookie (the UI uses `shareOneCap` `"وصلت إلى الحد الأقصى لروابط المشاركة في حسابك"`/`"You've reached the share-link limit on your account"` and `shareOneBusy` `"طلبات كثيرة بسرعة — انتظر دقيقة ثم أعد المحاولة"`/`"Too many requests — wait a minute, then try again"` for 409/429). `GET /api/share?id=` returns `{ id, title, messages, ts, one }` and the public page renders the `firas-project` message with `buildProjectCard` (`app.js:79860-79885`, `51509-51560`): `📁` + name + `"{n} ملفات · {kb} KB"`/`"{n} files · {kb} KB"`, buttons `"▶ معاينة مباشرة"`/`"▶ Live preview"`, `"⬇ تنزيل الفولدر (ZIP)"`/`"⬇ Download folder (ZIP)"`, `"⚡ افتح بفراس كود"`/`"⚡ Open in Firas Code"`, then a collapsible row per file (`📄 path  {kb} KB ▾`).

---

## 9. Download as ZIP (`app.js:75651-75659`, `50823-50851`)
...
- null for a drawing request (`DRAW_REQUEST` `draw|sketch|ارسم|إرسم|ارسملي|ارسم لي|رسم بياني|رسم دالة|رسم شكل|رسم مثلث|رسم دائرة|رسمة|رسمه|مخطّط|مخطط`) unless app words appear (`DRAW_AS_APP` `website|web app|web page|page|site|interactive|canvas|html|css|javascript|js|game|موقع|صفحة|تطبيق|تفاعل|لعبة`) or `CODE_SPEC` matches (`app.js:2797-2798`, `2805`);
...
Follow-ups (`codeFollowupSpec`, `app.js:2850-2870`): never in plan mode (`state.mode === "plan"` → null); if the last assistant turn is a code card and the new message is not a document request (`CODE_DOC_OVERRIDE` or `detectFileRequest`) nor a pure explanation (`اشرح|وضّح|فسّر|ما معنى|شنو يعني|شرح|explain|what does|how does it|why is` without a build verb) but matches `detectCodeRequest` or `CODE_FOLLOWUP` (`عدّل|عدل|تعديل|غيّر|غير|بدّل|بدل|أضف|اضف|اضيف|ضيف|احذف|أصلح|اصلح|صحّح|صحح|كمّل|كمل|أكمل|اكمل|استمر|واصل|زِد|زد|حسّن|حسن|طوّر|طور|اجعل|اجعله|خلّي|خلي|أعد|اعد كتابة|نفس الكود|لا يعمل|ما يعمل|لا يشتغل|ما يشتغل|مايشتغل|مو شغال|مش شغال|معطّل|خربان|توقف|يعلق|علق|فيه خطأ|فيه مشكلة|خطأ|مشكلة|edit|modif|chang|updat|add|remov|delet|fix|continu|improv|refactor|append|extend|rewrit|make it|same code|keep going|bug|error|broken|crash|doesn't work|does not work|not working|won't work/run/start/open/load|nothing happens|stopped working|stuck`), reuse the previous card's `{lang, ext, label, filename}`.
...
Three commits carry the ports: `55eee79` ("A night of capabilities … seventeen UI surfaces ported from 21st.dev" — the seventeen are counted across all four products, not Code alone), `92d72a9` ("Nine surfaces Firas Code did not have, ported into its own idiom from 21st.dev") and `1755fca` ("The last three Firas Code surfaces …": change gutter, status bar, shortcut sheet). Every one is a **model** port — the interaction and layout idea rebuilt in vanilla JS/CSS — never the React/Tailwind code (the site has no build step). Before `92d72a9` the workspace already had: tabs, dock, breadcrumbs, context menu, find/replace with regex, folding, fuzzy open (command palette), change log, diff view, split panes, focus mode. The nine that commit added: Problems panel, Version-history timeline, Editor preferences, Minimap, Multiple cursors + column selection, Indent guides + bracket-pair depth, Quick fixes, File-tree drag-and-drop, Run bar / task bar.

Every 21st.dev reference named in `app.js` comments, with the Code surface it became (the comment text is the authority on which component was studied):

| 21st.dev source (as named in the comment) | Firas Code surface | Cite |
| --- | --- | --- |
| "empty state" model | empty console / empty preview blocks (`cweConT`, `cwePrevT` strings in STR) | `app.js:832`, `1921` |
| "empty state with marquee" | home hero shelf `cwHeroStage` (slow marquee of the user's project files) | `app.js:61130-61300` |
| "Empty State with Marquee" (shadcnui-blocks/empty-state-04, #19377) | the in-IDE empty states (`cwe*` helpers: masked skeleton column, title, one sentence, two actions) | `app.js:63742` |
| AI Task List (educalvolpz/ai-task-list, demo 23793) | build console task board `cwStepUILive` (the six tasks of §3.7, self-drawing check marks) | `app.js:62760-63200` |
| "Interactive Logs Table" (#10635) | console toolbar: level chips carrying counts, text filter, clock toggle | `app.js:64249-64466` |
| "Status" (#25395, diceui) | run status pill `cwRunBar*` (`idle/run/ok/warn/fail`) | `app.js:64490-64620` |
| "CI/CD Build Log Viewer" (#24897, cnippet.dev) + "Subagent Tool" (#12380, Agent Elements) | run & task bar `cwTb*` (every runnable target with its own remembered verdict; the live row shows an elapsed clock and the last output line) | `app.js:64720-64760` |
| "Toast" (#24297, cnippet.dev) | notification deck `cwDeck*` (a stack, newest on top, max 4, desktop only) | `app.js:65302-65600` |
| "Great UI Mobile Mockup" (saurabh-2607) | preview device chassis (390×844 / 834×1112 bezels, rotate; hidden on phones) | `app.js:65802` |
| "Code block" (ayushmxxn) | open-file tabs `cwRenderTabs` with the per-tab copy button | `app.js:65858-65961` |
| "Flexnative Breadcrumb" (felipemenezes098) + SmartBreadcrumb `maxItems` | breadcrumb above the editor `cwCrumbRender` (folds by count, row scrolls) | `app.js:66007-66032` |
| "Edit Tool" (@serafimcloud, demo 12385) | quick-fix card `cwQf*` (proposed edit with Apply / Create file / Ignore) | `app.js:66500` |
| "Segmented Control" (@ddoemonn, demo 23552) | editor-preferences segmented controls (a `role=radiogroup` with roving tabindex; the thumb moves on `inset-inline-start`, never `translateX`, because the labels are Arabic inside an LTR shell) | `app.js:68253` |
| "Status" MODEL (@diceui, demo 25395) | status bar `cwStatusBar` / `cwUpdateStatus` (position, lines, selection, language, size, indent, encoding, EOL, save pill) | `app.js:68716-68815` |
| anchored-popover model (#23548) + tooltip-with-stats (#212) | the ONE status-bar popover frame (go-to-line / document statistics / encoding), two-column key/value grid so an Arabic label can sit next to an LTR number | `app.js:68983` |
| "Toolbar Dock" (ruixen.ui, demo 14092) | top-bar dock with a sliding tooltip rail and the overflow menu (`cwDockRun/Edit/Out/More` strings) | `app.js:69835` |
| Splitter (@anubra266 / Ark UI) | draggable pane seams (`firas_cw_split_v1`) | `app.js:70168` |
| "Morphing Dialog" (ibelick / motion-primitives, demo 1438) | focus mode (`zen`): one boolean promotes the editor to the whole workspace and returns it to the same box; Esc or the same button leaves | `app.js:70363` |
| Tree View by ddoemonn | find-results panel browsing (one flat roving list of visible rows, ←/→ shut and open a file group) | `app.js:71823` |
| "Chapter Scrubber" (@ruixen.ui, demo 19825) | minimap / document overview rail (`mapTitle`, ≥200 lines, desktop) | `app.js:72632` |
| FileTree | the file rail `cwRenderTree` | `app.js:73412-73614` |
| edwinvakayil/file-tree (demo 19150) | tree filter + roving keyboard on the rail (↑↓, ←→ fold, Home/End, `*` open all) | `app.js:73571` |
| originui/tree (#4322) + ddoemonn/reorder-list (#23550) | DnD selection grammar (click collapses to one, Ctrl/Cmd-click toggles, Shift-click ranges over visible rows) and the keyboard grab → move → drop → cancel with live-region announcements (`cwDnd*`) | `app.js:74097-74860` |
| ddoemonn/context-menu (demo 23560) | file/folder context menu `cwCtxItems` | `app.js:74817-74946` |
| "21st model" recents | command-palette recents (`firas_code_pal_recent_v1`, max 4, stable path ids) | `app.js:76548-76552` |
| @tom_ui/kbd (component 12235) | keyboard-shortcut sheet (`?`): one `<kbd>` cap per chord, the on-screen cap presses when the real chord is held | `app.js:76820-77003` |
| ToolCallsSection | Agent tool-call timeline — belongs to the Agent slice, **not** Code | `app.js:52259` |

Native priority: open-file tabs, breadcrumb (iPad), device chassis, console toolbar, run status pill, notification deck (or system toasts), version history, change log, diff review, status bar (position / size / save pill only). Defer: minimap, multi-cursor, indent guides, quick fixes, DnD, keyboard sheet, Problems panel, Firas Computer, pane splitter (use the system split view on iPad).
...
5. **Build**: `POST /api/chat/job` exactly as §3.1, poll per §3.3 (4 s, 2 h ceiling, dropped/non-403 failed polls never count, 15 consecutive "no such project" ticks before forgetting, a 403 forgets at once, land-before-forget with a 15 × 4 s grace after the terminal phase), pointer persisted per project (jobId, name, serverId, startedAt), reattach on foreground/launch/online, APNs deep link handling by jobId, checkpoints landed via `PUT /api/chats/:id` messages[0], toasts per §3.3.
...
- The existing `CodeStore.swift` charges a Code unit before enqueueing; the web does not for server builds. Harmless for members (unmetered), but it consumes one of a guest's 60 daily units (`ios/FirasAI/Stores/CodeStore.swift:295`).
- Every in-IDE model call (`nomem:true`) costs a guest one `internal` unit out of 300/day (§6.6); the native app must show the same daily-limit toast when `/api/chat` answers 429 with `quota.product === "internal"`, not a generic network error.
- The web's blank scaffold is Arabic/RTL even in the English UI (§2); keep that so a project created on the web and opened natively (or vice versa) looks identical.
